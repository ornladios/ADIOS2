...
.. rubric:: Remote HDF5 data in Campaign archives

When an HDF5 file is opened through the Campaign Reader, its original data
may reside in an HTTPS or S3 object, either as an individual file or as a
member of an indexed TAR object. In that case, ADIOS2 uses the metadata
surrogate embedded in the campaign for discovery and small embedded values.
Larger dataset values are read lazily from the original object using the
configured HTTPS or S3 transport and HDF5 selections.

This support belongs to the Campaign Reader workflow. It does not make an
HTTPS URL or S3 object key directly openable by passing that string to the
HDF5 engine. See :doc:`campaign` for the host configuration and build
requirements.

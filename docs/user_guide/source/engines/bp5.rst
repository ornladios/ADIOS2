...
#. Miscellaneous
   ...
   #. **Threads**: Read side: Specify how many threads one process can use to speed up reading in *PerformGets()/EndStep()*. You need to use Deferred mode in *Get()* calls to schedule reading variables to exploit parallel reading within the process but then it works for a single read selection as well if it spans multiple data blocks in the file. The default value is *0*, to let the engine estimate the number of threads based on how many processes are running on the compute node and how many hardware threads are available on the compute node but it will use maximum 16 threads. Value *1* forces the engine to read everything within the main thread of the process. Other values specify the exact number of threads the engine can use. 

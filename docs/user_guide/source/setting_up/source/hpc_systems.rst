...
#. **Modules:** Make sure all "module" dependencies are loaded and that minimum requirements are satisfied.
   ...
   Build with a C++17-compliant compiler, such as gcc >= 7, Intel >= 19.1, and PGI >= 19.
...
#. **PGI compilers and C++17 support:** Version 19 of the PGI compiler is C++17 compliant.
   However it relies on the C++ standard library headers supplied by the system version of GCC, which may or may not support all the C++17 features used in ADIOS2.
   On many systems, even though the PGI compiler supports C++17, the configured GCC and its headers may not.

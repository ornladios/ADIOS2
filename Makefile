...
MPI_HFiles=$(shell find ./include/file_mpi/ -type f -name "*MPI.h")
NoMPI_HFiles=$(shell find ./include/file_nompi/ -type f -name "*NoMPI.h")
HFiles=$(MPI_HFiles) $(NoMPI_HFiles)

#Source *.cpp Files and Object Files
MPI_CPPFiles=$(shell find ./src/file_mpi/ -type f -name "*MPI.cpp")
MPI_ObjFiles=$(patsubst ./src/file_mpi/%.cpp, ./bin/%.o, $(MPI_CPPFiles))

NoMPI_CPPFiles=$(shell find ./src/file_nompi/ -type f -name "*NoMPI.cpp")
NoMPI_ObjFiles=$(patsubst ./src/file_nompi/%.cpp, ./bin/%.o, $(NoMPI_CPPFiles))
...
./bin/%.o: ./src/file_mpi/%.cpp $(MPI_HFiles)
...
./bin/%.o: ./src/file_nompi/%.cpp $(NoMPI_HFiles)
	$(CC) $(CFLAGS) $(INCLUDE) -o $@ $<

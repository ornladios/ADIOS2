/*
 *   This file is not an official part of DILL, but is included temporarily for debugging purposes.  
 *   It is a modification of disassembler.c from https://github.com/btbd/disassembler.  (Modified to compile on windows.)  And is subject to the APACHE license.
 */
	...
	for (i = 0; i < instructions[opcode].argument_count; i++) {
		...
		switch (instructions[opcode].arguments[i]) {
			...
			case IMM32:
				...
				break;
			case REL8:
                                sprintf(output + strlen(output), "0x%x", (unsigned int) (offset + ((bytes - base) + 1) + *(char *)bytes));
                                bytes += 1;
				break;
			case REL32:

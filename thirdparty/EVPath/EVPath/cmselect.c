...
static void setup_wake_mechanism(CMtrans_services svcs, select_data_ptr *sdp);
...
static void init_select_data(svc, sdp, cm) CMtrans_services svc;
select_data_ptr *sdp;
CManager cm;
{
    ...
    *sdp = sd;
    ...
    sd->sel_item_max = 0;
    sd->select_items =
        (FunctionListElement *)svc->malloc_func(sizeof(FunctionListElement));
...
static void free_select_data(svc, sdp) CMtrans_services svc;
select_data_ptr *sdp;
{
...
static void set_soonest_timeout(timeout, task_list,
                                now) struct timeval *timeout;
...
static void increment_time(time, increment_sec,
                           increment_usec) struct timeval *time;
int increment_sec;
int increment_usec;
{
...
static void shutdown_wake_mechanism(select_data_ptr sd);

static void socket_select(svc, sd, timeout_sec,
                          timeout_usec) CMtrans_services svc;
select_data_ptr sd;
int timeout_sec;
int timeout_usec;
{
    ...
    if (sd->server_thread != thr_thread_self())
    {
        ...
        fprintf(stderr,
                "Warning:  Multiple threads calling CManager_socket_select.\n");
        fprintf(stderr, "          This situation may result in unexpected I/O "
                        "blocking.\n");
    ...
    if ((timeout_sec >= 0) || (sd->periodic_task_list != NULL))
    {
        ...
        svc->verbose(sd->cm, CMSelectVerbose,
                     "CMSelect with timeout %d sec, %d usec", timeout.tv_sec,
                     timeout.tv_usec);
        ...
        res = select(sd->sel_item_max + 1, &rd_set, &wr_set, (fd_set *)NULL,
                     &timeout);
    ...
    if (res == -1)
    {
         ...
         */
        if (sd->select_consistency_number != tmp_select_consistency_number)
            return;
        if (errno == 0)
        {
            /*
        ...
        if (errno == EBADF)
        {
            ...
            for (j = 0; j < FD_SETSIZE; j++)
            {
                if (FD_ISSET(j, &rd_set))
                {
                    ...
                    {
                        fprintf(stderr,
                                "Select failed, fd %d is bad.  Removing from "
                                "select list.\n",
                                j);
            ...
            /* if (found_one == 0) { fprintf(stderr, "Bad file descriptor in
             * select Warning.  Failed to localize.\n"); } */
        }
        else if (errno != EAGAIN)
        {
#ifdef HAVE_FDS_BITS
            fprintf(stderr,
                    "select failed, errno %d, rd_set was %lx, %lx,%lx, %lx\n\n",
                    errno, (long)((fd_set *)sd->fdset)->fds_bits[0],
...
#ifdef HAVE_FDS_BITS
    svc->verbose(sd->cm, CMSelectVerbose,
                 "select returned, rd_set started %lx, %lx, %lx, %lx, result "
                 "was %lx, %lx, %lx, %lx",
    ...
    svc->verbose(sd->cm, CMSelectVerbose,
                 "       write_set started %lx, %lx, %lx, %lx, result was %lx, "
                 "%lx, %lx, %lx",
    ...
    if (res != 0)
    {
        for (i = 0; i <= sd->sel_item_max; i++)
        {
            ...
            if (FD_ISSET(i, &wr_set))
            {
                ...
                {
                    svc->verbose(sd->cm, CMSelectVerbose,
                                 "Running select write action on fd %d", i);
            ...
            if (FD_ISSET(i, &rd_set))
            {
                ...
                {
                    svc->verbose(sd->cm, CMSelectVerbose,
                                 "Running select read action on fd %d", i);
                ...
                else
                {
                    ...
                    {
                        printf(
                            "FD %d is set in rd_set without being in fdset\n",
                            i);
                        assert(FALSE);
                    }
                }
...
extern void libcmselect_LTX_add_select(svc, sdp, fd, func, arg1,
                                       arg2) CMtrans_services svc;
select_data_ptr *sdp;
int fd;
select_list_func func;
void *arg1;
void *arg2;
{
    ...
    {
        fprintf(stderr,
                "The file descriptor number (%d) has exceeded the capability "
                "of select() on this system\n",
                fd);
#ifndef HAVE_WINDOWS_H
        fprintf(stderr, "Increase FD_SETSIZE if possible.\n");
#else
        fprintf(stderr,
                "Try running with a different control module if possible\n");
#endif
        fprintf(stderr, "Item not added to fdset.\n");
    }
    svc->verbose(sd->cm, CMSelectVerbose, "Adding fd %d to select read list",
                 fd);
...
extern void libcmselect_LTX_write_select(svc, sdp, fd, func, arg1,
                                         arg2) CMtrans_services svc;
select_data_ptr *sdp;
int fd;
select_list_func func;
...
void *arg2;
{
    ...
    {
        svc->verbose(sd->cm, CMSelectVerbose,
                     "Adding fd %d to select write list", fd);
        FD_SET(fd, (fd_set *)sd->write_set);
    }
    else
    {
        svc->verbose(sd->cm, CMSelectVerbose,
                     "Removing fd %d to select write list", fd);
...
extern periodic_task_handle
    libcmselect_LTX_add_periodic(svc, sdp, interval_sec, interval_usec, func,
                                 arg1, arg2) CMtrans_services svc;
select_data_ptr *sdp;
int interval_sec;
int interval_usec;
select_list_func func;
...
extern periodic_task_handle
    libcmselect_LTX_add_delayed_task(svc, sdp, delay_sec, delay_usec, func,
                                     arg1, arg2) CMtrans_services svc;
select_data_ptr *sdp;
int delay_sec;
int delay_usec;
...
static int remove_periodic_task(sd, handle) select_data_ptr sd;
periodic_task_handle handle;
{
...
extern void libcmselect_LTX_remove_periodic(svc, sdp,
                                            handle) CMtrans_services svc;
select_data_ptr *sdp;
periodic_task_handle handle;
{
    select_data_ptr sd = *((select_data_ptr *)sdp);
...
extern void libcmselect_LTX_remove_select(svc, sdp, fd) CMtrans_services svc;
select_data_ptr *sdp;
int fd;
{
    select_data_ptr sd = *((select_data_ptr *)sdp);
...
static void shutdown_wake_mechanism(sd) select_data_ptr sd;
{
...
static void read_wake_fd(fd_as_ptr, junk) void *fd_as_ptr;
void *junk;
{
    char buffer;
...
#ifdef HAVE_WINDOWS_H
static char *WSAerror_str(err) int err;
{
    switch (err)
    {
    case WSAEINTR:
...
int pipe(filedes) int filedes[2];
{
    ...
    if (bind(conn_sock, (struct sockaddr *)&sock_addr, sizeof sock_addr) ==
        SOCKET_ERROR)
    {
    ...
    if (connect(sock1, (struct sockaddr *)&sock_addr, sizeof sock_addr) ==
        SOCKET_ERROR)
    {
    ...
    if ((sock2 = accept(conn_sock, (struct sockaddr *)0, (int *)0)) ==
        SOCKET_ERROR)
    {
        int err = WSAGetLastError();
        printf("err was %s\n", WSAerror_str(err));
               ...
               sizeof(delay_value));
    {
        ...
        stTimeOut.tv_sec = 10;
        stTimeOut.tv_usec = 0;
        wRet = select(-1, NULL, (fd_set FAR *)&(stWriteFDS),
                      (fd_set FAR *)&(stXcptFDS), NULL);
...
static void setup_wake_mechanism(svc, sdp) CMtrans_services svc;
select_data_ptr *sdp;
{
    int filedes[2];
    ...
    svc->verbose(sd->cm, CMSelectVerbose,
                 "CMSelect Adding read_wake_fd as action on fd %d",
                 sd->wake_read_fd);
    libcmselect_LTX_add_select(svc, sdp, sd->wake_read_fd, read_wake_fd,
                               (void *)(long)sd->wake_read_fd, NULL);
}

extern void libcmselect_LTX_wake_function(svc, sdp) CMtrans_services svc;
select_data_ptr *sdp;
{
    if (*sdp != NULL)
    {
...
static void wake_server_thread(sd) select_data_ptr sd;
{
...
extern void libcmselect_LTX_blocking_function(svc,
                                              client_data) CMtrans_services svc;
void *client_data;
{
...
extern void libcmselect_LTX_polling_function(svc,
                                             client_data) CMtrans_services svc;
void *client_data;
{
    select_data_ptr sd = *((select_data_ptr *)client_data);
...
extern void libcmselect_LTX_select_initialize(svc, cm,
                                              client_data) CMtrans_services svc;
CManager cm;
void *client_data;
{
...
extern void libcmselect_LTX_select_shutdown(svc, cm,
                                            client_data) CMtrans_services svc;
CManager cm;
void *client_data;
{
...
extern void libcmselect_LTX_select_free(svc, cm,
                                        client_data) CMtrans_services svc;
CManager cm;
void *client_data;
{
...
extern void libcmselect_LTX_select_stop(svc, client_data) CMtrans_services svc;
void *client_data;
{
    if (*((select_data_ptr *)client_data) != NULL)
    {
...
extern void libcmselect_init_sel_item(struct _select_item *sel_item)
{
    ...
    sel_item->remove_periodic =
        (CMRemovePeriodicFunc)libcmselect_LTX_remove_periodic;

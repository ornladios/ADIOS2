...
static atom_t CM_ENET_V6ADDR = -1;
...
extern attr_list libcmenet_LTX_non_blocking_listen(CManager cm,
                                                   CMtrans_services svc,
                                                   transport_entry trans,
                                                   attr_list listen_info);

#ifndef USE_IPV6
static int check_host(char *hostname, void *sin_addr)
{
    (void)hostname;
    (void)sin_addr;
...
static enet_conn_data_ptr create_enet_conn_data(CMtrans_services svc)
{
...
static void *enet_accept_conn(enet_client_data_ptr ecd, transport_entry trans,
                              ENetAddress *address);
...
static void handle_packet(CManager cm, CMtrans_services svc,
                          transport_entry trans, enet_conn_data_ptr econn_d,
                          ENetPacket *packet)
{
    CMbuffer cb;
    ...
    cb = svc->create_data_and_link_buffer(cm, packet->data,
...
static void enet_service_network(CManager cm, void *void_trans)
{
    ...
    while (ecd->server && (enet_host_service(ecd->server, &event, 0) > 0))
    {
        ...
        {
            fprintf(stderr,
                    "WARNING, time between zero return for enet_host_service = "
                    "%d msecs\n",
                    enet_time_get() - ecd->last_host_service_zero_return);
        }
        ...
        case ENET_EVENT_TYPE_CONNECT:
        {
            ...
            inet_ntop(AF_INET6, &event.peer->address.host, straddr,
                      sizeof(straddr));
            svc->trace_out(cm, "A new client connected from %s:%u.\n",
                           &straddr[0], event.peer->address.port);
#endif

            enet_connection_data =
                enet_accept_conn(ecd, trans, &event.peer->address);

            /* Store any relevant client information here. */
            svc->trace_out(cm,
                           "ENET ========   Assigning peer %p has data %p\n",
        ...
        case ENET_EVENT_TYPE_RECEIVE:
        {
            ...
            else
            {
                ...
                addr.s_addr = event.peer->address.host;
                svc->trace_out(
                    cm, "ENET  ====== virgin peer, address is %s, port %u.\n",
                    inet_ntoa(addr), event.peer->address.port);
#else
                char straddr[INET6_ADDRSTRLEN];
                inet_ntop(AF_INET6, &event.peer->address.host, straddr,
...
static void enet_service_network_lock(CManager cm, void *void_trans)
{
    transport_entry trans = (transport_entry)void_trans;
    enet_client_data_ptr ecd = (enet_client_data_ptr)trans->trans_data;
    CMtrans_services svc = ecd->svc;
...
static void read_wake_fd_and_service(CManager cm, void *void_trans)
{
    transport_entry trans = (transport_entry)void_trans;
    enet_client_data_ptr ecd = (enet_client_data_ptr)trans->trans_data;
...
#ifdef NOTDEF
static void dump_sockaddr(who, sa) char *who;
struct sockaddr_in *sa;
{
    ...
    printf("%s: family=%d port=%d addr=%d.%d.%d.%d\n", who,
           ntohs(sa->sin_family), ntohs(sa->sin_port), addr[0], addr[1],
           addr[2], addr[3]);
}

static void dump_sockinfo(msg, fd) char *msg;
int fd;
{
    int nl;
                              ...
                              ENetAddress *address)
{
    ...
    enet_conn_data->remote_IP =
        ntohl(address->host); /* remote_IP is in host byte order */
#else
    ...
    if (enet_conn_data->remote_host != NULL)
    {
        svc->trace_out(trans->cm,
                       "Accepted ENET RUDP connection from host \"%s\"",
                       enet_conn_data->remote_host);
    }
    else
    {
        svc->trace_out(trans->cm,
                       "Accepted ENET RUDP connection from UNKNOWN host");
    }
    ...
    svc->trace_out(trans->cm, "Remote host (IP %s) is listening at port %d\n",
                   inet_ntoa(addr), enet_conn_data->remote_contact_port);
#else
    char straddr[INET6_ADDRSTRLEN];
    inet_ntop(AF_INET6, &enet_conn_data->remote_IP, straddr, sizeof(straddr));
    svc->trace_out(trans->cm, "Remote host (IP %s) is listening at port %d\n",
                   &straddr[0], enet_conn_data->remote_contact_port);
#endif
...
extern void libcmenet_LTX_shutdown_conn(CMtrans_services svc,
...
static int initiate_conn(CManager cm, CMtrans_services svc,
                         ...
                         attr_list conn_attr_list)
{
    ...
    {
        svc->trace_out(cm,
                       "CMEnet transport found no CM_ENET_HOSTNAME attribute");
        host_name = NULL;
    }
    else
    {
    ...
    {
        svc->trace_out(cm,
                       "CMEnet transport found no CM_ENET_V6ADDR attribute");
        /* wasn't there */
        if (host_name == NULL)
            printf("No host no IP\n");
        return 0;
    ...
    else
    {
        ...
        {
            svc->trace_out(cm, "CMEnet transport length was wrong in "
                               "CM_ENET_V6ADDR attribute");
            if (host_name == NULL)
                printf("No host no IP\n");
            return 0;
        }
        else
        {
            ...
            inet_ntop(AF_INET6, &host_ip, straddr, sizeof(straddr));
            svc->trace_out(cm, "CMEnet transport connect to host_IP %s",
                           &straddr[0]);
        }
    }
#endif
    ...
    {
        svc->trace_out(
            cm, "CMEnet transport found no CM_ENET_CONN_TIMEOUT attribute");
    }
    else
    {
        svc->trace_out(
            cm, "CMEnet transport connection timeout set to %d msecs", timeout);
    }
    if (!query_attr(attrs, CM_ENET_CONN_REUSE, /* type pointer */ NULL,
                    /* value pointer */ (attr_value *)(long)&conn_reuse))
    {
        svc->trace_out(
            cm, "CMEnet transport found no CM_ENET_CONN_REUSE attribute");
    }
    else
    {
        svc->trace_out(cm, "CMEnet transport connection reuse set to %d",
                       conn_reuse);
    }
    ...
    if (host_name)
    {
        ...
        svc->trace_out(cm,
                       "Attempting ENET RUDP connection, USING host=\"%s\", IP "
                       "= %s, port %d",
                       host_name == 0 ? "(unknown)" : host_name,
                       inet_ntoa(sin_addr), int_port_num);
#else
        char straddr[INET6_ADDRSTRLEN];
        inet_ntop(AF_INET6, &event.peer->address.host, straddr,
                  sizeof(straddr));
        svc->trace_out(cm,
                       "Attempting ENET RUDP connection, USING host=\"%s\", IP "
                       "= %s, port %d",
                       host_name == 0 ? "(unknown)" : host_name, &straddr[0],
                       int_port_num);
#endif
    }
    else
    {
        ...
        svc->trace_out(
            cm, "Attempting ENET RUDP connection, USING IP = %s, port %d",
            inet_ntoa(sin_addr), int_port_num);
#else
        char straddr[INET6_ADDRSTRLEN];
        inet_ntop(AF_INET6, &host_ip, straddr, sizeof(straddr));
    ...
    svc->trace_out(cm,
                   "ENET ========   On init Assigning peer %p has data %p\n",
                   peer, enet_conn_data);
    if (peer == NULL)
    {
        fprintf(stderr,
    ...
    while (!finished)
    {
        ...
        case ENET_EVENT_TYPE_CONNECT:
        {
            if (event.peer != peer)
            {
                ...
                /* Store any relevant client information here. */
                svc->trace_out(
                    cm, "ENET ========   Assigning peer %p has data %p\n",
                    event.peer, enet_connection_data);
                event.peer->data = enet_connection_data;
                ((enet_conn_data_ptr)enet_connection_data)->peer = event.peer;
                enet_host_flush(ecd->server);
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
            if (event.peer == peer)
            {
                ...
                svc->trace_out(cm,
                               "Connection to %s:%d failed   type was %d.\n",
...
#else
    memcpy(&enet_conn_data->remote_IP, &host_ip,
           sizeof(enet_conn_data->remote_IP));
#endif
 ...
 */
extern CMConnection libcmenet_LTX_initiate_conn(CManager cm,
                                                CMtrans_services svc,
                                                transport_entry trans,
                                                attr_list attrs)
{
 ...
 */
extern int libcmenet_LTX_self_check(CManager cm, CMtrans_services svc,
                                    transport_entry trans, attr_list attrs)
{
    ...
    get_IP_config(my_host_name, sizeof(host_name), &IP, NULL, NULL, NULL, NULL,
    ...
    if (!query_attr(attrs, CM_ENET_HOSTNAME, /* type pointer */ NULL,
                    /* value pointer */ (attr_value *)(long)&host_name))
    {
        svc->trace_out(cm, "CMself check CMEnet transport found no "
                           "CM_ENET_HOSTNAME attribute");
        host_name = NULL;
    }
    if (!query_attr(attrs, CM_ENET_ADDR, /* type pointer */ NULL,
                    /* value pointer */ (attr_value *)(long)&host_addr))
    {
        svc->trace_out(
            cm,
            "CMself check CMEnet transport found no CM_ENET_ADDR attribute");
        if (host_name == NULL)
            return 0;
        host_addr = 0;
    }
                    ...
                    /* value pointer */ (attr_value *)(long)&int_port_num))
    {
        svc->trace_out(
            cm,
            "CMself check CMEnet transport found no CM_ENET_PORT attribute");
        return 0;
    }
...
extern int libcmenet_LTX_connection_eq(CManager cm, CMtrans_services svc,
                                       transport_entry trans, attr_list attrs,
                                       enet_conn_data_ptr ecd)
{
    ...
    {
        svc->trace_out(
            cm, "Conn Eq CMenet transport found no CM_ENET_PORT attribute");
        return 0;
    }
#ifndef USE_IPV6
    if (!query_attr(attrs, CM_ENET_ADDR, /* type pointer */ NULL,
    ...
    svc->trace_out(cm, "ENET Conn_eq comparing IP/ports %s/%d and %s/%d",
                   inet_ntoa(addr1), ecd->remote_contact_port, inet_ntoa(addr2),
                   int_port_num);
    ...
    inet_ntop(AF_INET6, &ecd->remote_IP, straddr1, sizeof(straddr1));
    inet_ntop(AF_INET6, &v6_request, straddr2, sizeof(straddr2));

    svc->trace_out(cm, "ENET Conn_eq comparing IP/ports %s/%d and %s/%d",
                   straddr1, ecd->remote_contact_port, straddr2, int_port_num);

    if ((in6_equal(ecd->remote_IP, requested_IP)) &&
        (ecd->remote_contact_port == int_port_num))
    {
        svc->trace_out(cm, "ENET Conn_eq returning TRUE");
        return 1;
...
static attr_list build_listen_attrs(CManager cm, CMtrans_services svc,
                                    enet_client_data_ptr ecd,
                                    attr_list listen_info, int int_port_num)
{
    ...
    svc->trace_out(cm, "CMEnet listen succeeded on port %d", int_port_num);
#ifndef USE_IPV6
    get_IP_config(host_name, sizeof(host_name), &IP, NULL, NULL, &use_hostname,
                  listen_info, svc->trace_out, (void *)cm);
#else
    // for the moment, just use ipv4 in the ipv6 address
    //    get_IPv6_config(host_name, sizeof(host_name), &IP, (char*)&IPV6, NULL,
    //    NULL,
    ...
    ((enet_uint32 *)&IPV6.s6_addr)[2] = htonl(0xffff);
    ((enet_uint32 *)&IPV6.s6_addr)[3] = htonl(IP);
#endif
    ...
    {
        add_attr(ret_list, CM_ENET_ADDR, Attr_Int4, (attr_value)(long)IP);
    }
#else
    if ((IP != 0) && !use_hostname)
    {
...
#endif
    if ((getenv("CMEnetsUseHostname") != NULL) || use_hostname)
    {
    ...
    add_attr(ret_list, CM_ENET_PORT, Attr_Int4, (attr_value)(long)int_port_num);

    add_attr(ret_list, CM_TRANSPORT, Attr_String, (attr_value)strdup("enet"));
    return ret_list;
}

static void wake_enet_server_thread(enet_client_data_ptr enet_data)
{
                                                   ...
                                                   attr_list listen_info)
{
     ...
     */
    if (listen_info != NULL && !query_attr(listen_info, CM_ENET_PORT, NULL,
                                           (attr_value *)(long)&attr_port_num))
    {
        port_num = 0;
    }
    else
    ...
    if (enet_data->server != NULL)
    {
        ...
        if (port_num == 0)
        {
            ...
            return build_listen_attrs(cm, svc, NULL, listen_info,
                                      enet_data->listen_port);
        }
        else
        {
            printf("CMlisten_specific() requesting a specific port follows "
                   "other Enet operation which initiated listen at another "
                   "port.  Only one listen allowed, second listen fails.\n");
            return NULL;
        }
    }
    if (port_num != 0)
    ...
    else
    {
        ...
        int tries;
        srand48(seedval);
        get_IP_config(NULL, 0, NULL, &low_bound, &high_bound, NULL, listen_info,
                      svc->trace_out, (void *)cm);
    ...
    svc->fd_add_select(cm, enet_host_get_sock_fd(server),
                       (select_list_func)enet_service_network, (void *)cm,
                       (void *)trans);

    svc->add_periodic_task(cm, 0, 100, (CMPollFunc)enet_service_network_lock,
                           (void *)trans);

    svc->trace_out(enet_data->cm,
                   "CMENET Adding read_wake_fd as action on fd %d",
                   enet_data->wake_read_fd);

    svc->fd_add_select(cm, enet_data->wake_read_fd,
                       (select_list_func)read_wake_fd_and_service, (void *)cm,
                       (void *)trans);
...
extern void *libcmenet_LTX_read_block_func(CMtrans_services svc,
...
extern int libcmenet_LTX_writev_func(CMtrans_services svc,
                                     enet_conn_data_ptr ecd, struct iovec *iov,
                                     int iovcnt, attr_list attrs)
{
    ...
    ENetPacket *packet =
        enet_packet_create(NULL, length, ENET_PACKET_FLAG_RELIABLE);
    ...
    if (enet_peer_send(ecd->peer, 0, packet) == -1)
    {
        ...
        svc->trace_out(
            ecd->ecd->cm,
            "ENET  ======  failed to send a packet to peer %p, state %d\n",
...
static void free_enet_data(CManager cm, void *ecdv)
{
...
static void shutdown_enet_thread(CManager cm, void *ecdv)
{
    enet_client_data_ptr ecd = (enet_client_data_ptr)ecdv;
    CMtrans_services svc = ecd->svc;
    (void)cm;
...
extern void *libcmenet_LTX_initialize(CManager cm, CMtrans_services svc,
                                      transport_entry trans, attr_list attrs)
{
    ...
    if (atom_init == 0)
    {
        ...
        CM_ENET_V6ADDR = attr_atom_from_string("CM_ENET_v6ADDR");
    ...
    if (env)
    {
        ...
        fprintf(stderr,
                "DEBUG: Setting enet_host_service_warn_interval to %d\n",
                enet_host_service_warn_interval);
    }
    ...
    svc->add_shutdown_task(cm, shutdown_enet_thread, (void *)enet_data,
                           SHUTDOWN_TASK);
                                                   ...
                                                   CMtrans_services svc)
{
    ...
    transport->listen =
        (CMTransport_listen_func)libcmenet_LTX_non_blocking_listen;
    ...
    transport->shutdown_conn =
        (CMTransport_shutdown_conn_func)libcmenet_LTX_shutdown_conn;
    transport->read_block_func =
        (CMTransport_read_block_func)libcmenet_LTX_read_block_func;

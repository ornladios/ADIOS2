...
static int get_self_ipv6_iface(struct in6_addr *addr_buf,
                               CMTransport_trace trace_func, void *trace_data,
                               char *interface)
{
    ...
    if (getifaddrs(&if_addrs) == 0)
    {
        ...
        for (if_addr = if_addrs; if_addr != NULL; if_addr = if_addr->ifa_next)
        {
            ...
            {
                //		dump_output(1023, "\t" IPCONFIG_ENVVAR_PREFIX
                //"IP_CONFIG Possible interface %s : IPV4 %s\n",
                //			    if_addr->ifa_name,
                //			    inet_ntop(family, tmp, buf,
                //sizeof(buf)));
            }
            else
            {
                dump_output(1023,
                            "\t" IPCONFIG_ENVVAR_PREFIX
                            "IP_CONFIG Possible interface %s : IPV6 %s\n",
                            if_addr->ifa_name,
                            inet_ntop(family, tmp, buf, sizeof(buf)));
            }
        }
        ...
        if (interface != NULL)
        {
            ...
            if (first_call)
                dump_output(1023,
                            "\t" IPCONFIG_ENVVAR_PREFIX
                            "IP_CONFIG interface %s requested\n",
                            interface);
                 ...
                 if_addr = if_addr->ifa_next)
            {
                ...
                family = if_addr->ifa_addr->sa_family;
                if (family != AF_INET6)
                    continue; /* currently looking for ipv6 */
                if (strncmp(if_addr->ifa_name, interface, strlen(interface)) !=
                    0)
                    continue;
                tmp = &((struct sockaddr_in6 *)if_addr->ifa_addr)->sin6_addr;
                trace_func(
                    trace_data,
                    "CM<IP_CONFIG> Interface specified, returning ->%s : %s",
                    if_addr->ifa_name,
                    inet_ntop(family, tmp, buf, sizeof(buf)));
                if (first_call)
                    dump_output(1023,
            ...
            printf("Warning!  " IPCONFIG_ENVVAR_PREFIX
                   "INTERFACE specified as \"%s\", but no active interface by "
                   "that name found\n",
                   interface);
        }
            ...
            for (p = host->h_addr_list; *p != 0; p++)
            {
                ...
                if (!IN6_IS_ADDR_LOOPBACK(in))
                {
                    ...
                    trace_func(trace_data,
                               "CM<IP_CONFIG> Prefer IP associated with "
                               "hostname net -> %s",
                               str);
        ...
        for (if_addr = if_addrs; if_addr != NULL; if_addr = if_addr->ifa_next)
        {
            ...
            if ((if_addr->ifa_flags & IFF_LOOPBACK) != 0)
                continue;
            tmp = &((struct sockaddr_in6 *)if_addr->ifa_addr)->sin6_addr;
            if (IN6_IS_ADDR_LOOPBACK((struct in6_addr *)tmp))
                continue;
            if (IN6_IS_ADDR_SITELOCAL((struct in6_addr *)tmp))
                continue;
            if (IN6_IS_ADDR_LINKLOCAL((struct in6_addr *)tmp))
                continue;
            trace_func(trace_data,
                       "CM<IP_CONFIG> get_self_ipv6_addr returning first avail "
                       "-> %s : %s",
                       if_addr->ifa_name,
                       inet_ntop(family, tmp, buf, sizeof(buf)));
            memcpy(addr_buf, tmp, sizeof(struct in6_addr));
...
static int get_self_ip_iface(CMTransport_trace trace_func, void *trace_data,
                             char *interface)
{
    struct hostent *host = NULL;
    char hostname_buf[256];
    ...
    if (getifaddrs(&if_addrs) == 0)
    {
        ...
        for (if_addr = if_addrs; if_addr != NULL; if_addr = if_addr->ifa_next)
        {
            ...
            else
            {
                ...
                // dump_output(1023, "\t" IPCONFIG_ENVVAR_PREFIX "IP_CONFIG
                // Possible interface %s : IPV6 %s\n", 	    if_addr->ifa_name,
                //	    inet_ntop(family, tmp, buf, sizeof(buf)));
            }
        }
        if (!interface)
        ...
        if (interface != NULL)
        {
                 ...
                 if_addr = if_addr->ifa_next)
            {
                ...
                family = if_addr->ifa_addr->sa_family;
                if (family != AF_INET)
                    continue; /* currently not looking for ipv6 */
                if (strncmp(if_addr->ifa_name, interface, strlen(interface)) !=
                    0)
                    continue;
                tmp = &((struct sockaddr_in *)if_addr->ifa_addr)->sin_addr;
        ...
        for (if_addr = if_addrs; if_addr != NULL; if_addr = if_addr->ifa_next)
        {
            ...
            tmp = &((struct sockaddr_in *)if_addr->ifa_addr)->sin_addr;
            trace_func(trace_data,
                       "CM<IP_CONFIG> get_self_ip_addr returning first avail "
                       "-> %s : %s",
                       if_addr->ifa_name,
                       inet_ntop(family, tmp, buf, sizeof(buf)));
    ...
    if (host != NULL)
    {
        for (p = host->h_addr_list; *p != 0; p++)
        {
            ...
            if (!ipv4_is_loopback(ntohl(in->s_addr)))
            {
                ...
                inet_ntop(AF_INET, &(in->s_addr), str, sizeof(str));
                trace_func(trace_data,
                           "CM<IP_CONFIG> - Get self IP addr %lx, net %s",
                           ntohl(in->s_addr), str);
                return (ntohl(in->s_addr));
            }
        }
    ...
    if (ioctl(ss, SIOCGIFCONF, &ifaces) >= 0)
    {
        ...
        for (; ifrn--; ifr++)
        {
            ...
            if (ifr->ifr_flags & IFF_LOOPBACK)
            {
                trace_func(
                    trace_data,
                    "CM<IP_CONFIG> - Get self IP addr %s, rejected, loopback",
            ...
            if (!(ifr->ifr_flags & IFF_UP))
            {
                trace_func(
                    trace_data,
                    "CM<IP_CONFIG> - Get self IP addr %lx, rejected, not UP",
            ...
            {
                trace_func(trace_data,
                           "CM<IP_CONFIG> - Get self IP addr %lx, rejected, "
                           "not RUNNING",
                           ntohl(sai->sin_addr.s_addr));
                continue;
            }
    ...
    if (rv == 0)
    {
        ...
        trace_func(trace_data,
                   "CM<IP_CONFIG> - Get self IP addr at last resort");
        if (c != NULL)
        {
            trace_func(trace_data, "CM<IP_CONFIG> - Translating last resort %s",
                       c);
            rv = inet_addr(c);
        }
    }
...
static int get_self_ip_addr(CMTransport_trace trace_func, void *trace_data)
{
    return get_self_ip_iface(trace_func, trace_data, NULL);
}

static int get_self_ipv6_addr(struct in6_addr *addr_buf,
                              CMTransport_trace trace_func, void *trace_data)
{
    return get_self_ipv6_iface(addr_buf, trace_func, trace_data, NULL);
}

static int is_private_IP(int IP)
{
    if ((IP & 0xffff0000) == 0xC0A80000)
        return 1; /* equal 192.168.x.x */
    if ((IP & 0xffff0000) == 0xB6100000)
        return 1; /* equal 182.16.x.x */
    if ((IP & 0xff000000) == 0x0A000000)
        return 1; /* equal 10.x.x.x */
    return 0;
}

static void get_qual_hostname(char *buf, int len, attr_list attrs,
                              int *network_p, CMTransport_trace trace_func,
                              void *trace_data)
{
    struct hostent *host = NULL;
    ...
    trace_func(trace_data, "CM<IP_CONFIG> - Tentative Qualified hostname %s",
               buf);
    if (memchr(buf, '.', strlen(buf)) == NULL)
    {
    ...
    if (host != NULL)
    {
        ...
        for (p = host->h_addr_list; *p != 0; p++)
        {
            ...
            if (!ipv4_is_loopback(ntohl(in->s_addr)))
            {
                ...
                inet_ntop(AF_INET, &(in->s_addr), str, sizeof(str));
                trace_func(trace_data,
                           "CM<IP_CONFIG> - Hostname gets good addr %lx, %s",
                           ntohl(in->s_addr), str);
            }
        }
        if (good_addr == 0)
    ...
    if (buf[0] == 0)
    {
        ...
        trace_func(
            trace_data,
            "CM<IP_CONFIG> - No hostname yet, trying gethostbyaddr on IP %lx",
            IP);
    ...
    if (network_string == NULL)
    {
        ...
        {
            CM_NETWORK_POSTFIX =
                attr_atom_from_string(IPCONFIG_ENVVAR_PREFIX "NETWORK_POSTFIX");
        }
        if (!get_string_attr(attrs, CM_NETWORK_POSTFIX, &network_string))
        {
            trace_func(trace_data,
                       "TCP/IP transport found no NETWORK POSTFIX attribute");
        }
        else
        {
            trace_func(trace_data,
                       "TCP/IP transport found NETWORK POSTFIX attribute %s",
                       network_string);
        }
    }
    if (network_string != NULL)
        ...
        (memchr(buf, '.', strlen(buf)) == NULL))
    {
        ...
        else
        {
            ...
            if (warn_once == 0)
            {
                warn_once++;
                trace_func(trace_data,
                           "Attempts to establish your fully qualified "
                           "hostname, or indeed any\nuseful network name, have "
                           "failed horribly.  using localhost.\n");
            }
...
extern char *IP_get_diagnostics(CManager cm, CMTransport_trace trace_out)
{
    char *output;
    IP_config_output_len = 0; /* setup output */
    get_IP_config(NULL, 0, NULL, NULL, NULL, NULL, NULL, trace_out, cm);
...
static void dump_output(int length_estimate, char *format, ...)
{
    ...
    IP_config_diagnostics = realloc(IP_config_diagnostics,
                                    IP_config_output_len + length_estimate + 1);
...
#endif
extern void get_IPv6_config(char *hostname_buf, int len, int *IP_p,
                            char *IPv6_buf, int *port_range_low_p,
                            int *port_range_high_p, int *use_hostname_p,
                            attr_list attrs, CMTransport_trace trace_func,
                            void *trace_data)
{
    static int first_call = 1;
    ...
    if (first_call)
    {
        ...
        if (preferred_IP != NULL)
        {
            ...
            if (preferred_hostname)
                printf("Warning, " IPCONFIG_ENVVAR_PREFIX
                       "HOSTNAME and " IPCONFIG_ENVVAR_PREFIX
                       "IP are both set, preferring " IPCONFIG_ENVVAR_PREFIX
                       "IP\n");
            if (inet_aton(preferred_IP, &addr) == 0)
            {
                fprintf(
                    stderr,
                    "Invalid address %s specified for " IPCONFIG_ENVVAR_PREFIX
                    "IP\n",
                    preferred_IP);
            }
            else
            {
                trace_func(
                    trace_data,
                    "CM IP_CONFIG Using IP specified in " IPCONFIG_ENVVAR_PREFIX
                    "IP, %s",
                    preferred_IP);
                determined_IP = (ntohl(addr.s_addr));
                dump_output(1023,
                            "\t" IPCONFIG_ENVVAR_PREFIX
                            "IP environment variable found, preferring IP %s\n",
                            preferred_IP);
            }
        }
        else if (preferred_hostname != NULL)
        {
            ...
            trace_func(
                trace_data,
                "CM<IP_CONFIG> CM_HOSTNAME set to \"%s\", running with that.",
                preferred_hostname);
            dump_output(1023,
                        "\t" IPCONFIG_ENVVAR_PREFIX
                        "HOSTNAME environment variable found, trying \"%s\"\n",
                        preferred_hostname);
            ...
            {
                printf("Warning, " IPCONFIG_ENVVAR_PREFIX
                       "HOSTNAME is \"%s\", but gethostbyname fails for that "
                       "string.\n",
                       preferred_hostname);
                dump_output(
                    1023,
                    "\t" IPCONFIG_ENVVAR_PREFIX
                    "HOSTNAME \"%s\" fails to translate to IP address.\n",
                    preferred_hostname);
            }
            else
            {
                ...
                for (p = host->h_addr_list; *p != 0; p++)
                {
                    ...
                    if (!ipv4_is_loopback(ntohl(in->s_addr)))
                    {
                        ...
                        dump_output(
                            1023,
                            "\t"
                            "HOSTNAME \"%s\" translates to IP address %s.\n",
                ...
                if (determined_IP == -1)
                {
                    dump_output(1023,
                                "\t"
                                "No non-loopback interfaces found for hostname "
                                "\"%s\", rejected for IP use.\n",
                                preferred_hostname);
                }
            }
        }
        ...
        {
            get_qual_hostname(determined_hostname,
                              sizeof(determined_hostname) - 1, NULL /* attrs */,
                              NULL, trace_func, trace_data);
            dump_output(1023,
                        "\t" IPCONFIG_ENVVAR_PREFIX
                        "IP_CONFIG best guess hostname is \"%s\"\n",
                        determined_hostname);
        }
        if (determined_IP == -1)
        {
            /* I.E. the specified hostname didn't determine what IP we should
             * use */
            char str[INET_ADDRSTRLEN];
            struct in_addr addr;
        ...
        if (port_range != NULL)
        {
            if (sscanf(port_range, "%d:%d", &port_range_high,
                       &port_range_low) != 2)
            {
                printf(IPCONFIG_ENVVAR_PREFIX
                       "PORT_RANGE spec not understood \"%s\"\n",
                       port_range);
            }
            else
            {
                ...
                dump_output(1023,
                            "\t" IPCONFIG_ENVVAR_PREFIX
                            "IP_CONFIG specified port range is %d:%d\n",
        ...
        else
        {
            dump_output(1023,
                        "\t" IPCONFIG_ENVVAR_PREFIX
                        "IP_CONFIG default port range is %d:%d\n",
                        port_range_low, port_range_high);
        }
    }
    ...
    if (get_string_attr(attrs, CM_IP_INTERFACE, &interface))
    {
        ...
        get_qual_hostname(hostname_to_use, sizeof(hostname_to_use) - 1, attrs,
                          NULL, trace_func, trace_data);
    ...
    }
    {
        ...
        trace_func(trace_data,
                   "CM<IP_CONFIG> returning hostname \"%s\", IP %s, "
                   "use_hostname = %d, port range %d:%d",
                   hostname_to_use,
                   inet_ntop(AF_INET, &net_byte_order, &buf[0], 256),
                   use_hostname, port_range_low, port_range_high);
    }
}

extern void get_IP_config(char *hostname_buf, int len, int *IP_p,
                          int *port_range_low_p, int *port_range_high_p,
                          int *use_hostname_p, attr_list attrs,
                          CMTransport_trace trace_func, void *trace_data)
{
    get_IPv6_config(hostname_buf, len, IP_p, NULL, port_range_low_p,
                    port_range_high_p, use_hostname_p, attrs, trace_func,
                    trace_data);
}

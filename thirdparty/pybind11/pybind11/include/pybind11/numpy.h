...
/* This will be true on all flat address space platforms and allows us to reduce
   the
   whole npy_intp / ssize_t / Py_intptr_t business down to just ssize_t for all
   size
...
using is_pod_struct =
    ...
    defined(_GLIBCXX_USE_CXX11_ABI)
           ...
           // GCC 5 or newer, independent
           // of the actual compiler (Clang can also use libstdc++, but it
           // always defines __GNUC__ == 4).
 ...
 * constructed through
 * the `unchecked<T, N>()` method of `array` or the `unchecked<N>()` method of
 * `array_t<T>`.  `Dims`
...
public:
     ...
     * compile-time known
     * number of dimensions, this requires the correct number of arguments; for
     * run-time
     * dimensionality, this is not checked (and so is up to the caller to use
     * safely).
     */
    ...
    {
        static_assert(
            ssize_t{sizeof...(Ix)} == Dims || Dynamic,
     ...
     * if the reference
     * is to a 1-dimensional array.  When present, this is exactly equivalent to
     * `obj(index)`.
     */
    ...
    /// that the actual span in
    /// memory may be larger if the referenced array has non-contiguous strides
    /// (e.g. for a slice).
...
public:
     ...
     * participates if the
     * reference is to a 1-dimensional array (or has runtime dimensions).  When
     * present, this is
...
public:
    ...
    explicit dtype(const buffer_info &info)
    {
        ...
        m_ptr =
            descr
                .strip_padding(info.itemsize ? info.itemsize : descr.itemsize())
                .release()
                .ptr();
    }
    ...
    static dtype from_args(object args)
    {
        ...
        if (!detail::npy_api::get().PyArray_DescrConverter_(
                args.release().ptr(), &ptr) ||
            !ptr)
    ...
    dtype strip_padding(ssize_t itemsize)
    {
        ...
        for (auto field : attr("fields").attr("items")())
        {
            ...
            field_descriptors.push_back(
                {(PYBIND11_STR_TYPE)name,
                 format.strip_padding(format.itemsize()), offset});
        }
...
public:
    ...
    array(ShapeContainer shape, StridesContainer strides, const T *ptr,
          handle base = handle())
    : array(pybind11::dtype::of<T>(), std::move(shape), std::move(strides), ptr,
            base)
    {
    }
    ...
    {
        return detail::check_flags(m_ptr,
     ...
     * bounds or
     * dimensionality checking.  Will throw if the array is missing the
     * `writeable` flag.  Use with
     * care: the array must not be destroyed or reshaped for the duration of the
     * returned object,
     * and the caller must take care not to access invalid dimensions or
     * dimension indices.
     */
     ...
     * without bounds or
     * dimensionality checking.  Unlike `mutable_unchecked()`, this does not
     * require that the
     * underlying array have the `writable` flag.  Use with care: the array must
     * not be destroyed or
     * reshaped for the duration of the returned object, and the caller must
     * take care not to access
    ...
    void resize(ShapeContainer new_shape, bool refcheck = true)
    {
        ...
        object new_array =
            reinterpret_steal<object>(detail::npy_api::get().PyArray_Resize_(
                m_ptr, &d, int(refcheck), -1));
        if (!new_array)
            throw error_already_set();
...
protected:
    ...
    {
        throw index_error(msg + ": " + std::to_string(dim) + " (ndim = " +
                          std::to_string(ndim()) + ")");
    }
...
public:
    ...
    explicit array_t(size_t count, const T *ptr = nullptr,
                     handle base = handle())
    : array({count}, {}, ptr, base)
    {
    }
     ...
     * Returns a proxy object that provides const access to the array's data
     * without bounds or
     * dimensionality checking.  Unlike `unchecked()`, this does not require
     * that the underlying
     * array have the `writable` flag.  Use with care: the array must not be
     * destroyed or reshaped
     * for the duration of the returned object, and the caller must take care
     * not to access invalid
    ...
    /// not, try to convert
    /// it).  In case of an error, nullptr is returned and the Python error is
    /// cleared.
                         ...
                         detail::enable_if_t<detail::array_info<T>::is_array>>
{
    ...
    {
        using detail::_;
        PYBIND11_DESCR extents =
            _("(") + detail::array_info<T>::extents() + _(")");
        return extents.text() +
...
private:
    ...
    constexpr static const int values[15] = {
        npy_api::NPY_BOOL_,       npy_api::NPY_BYTE_,
        npy_api::NPY_UBYTE_,      npy_api::NPY_SHORT_,
        npy_api::NPY_USHORT_,     npy_api::NPY_INT_,
        npy_api::NPY_UINT_,       npy_api::NPY_LONGLONG_,
        npy_api::NPY_ULONGLONG_,  npy_api::NPY_FLOAT_,
        npy_api::NPY_DOUBLE_,     npy_api::NPY_LONGDOUBLE_,
...
public:
    ...
    template <typename T2 = T,
              enable_if_t<std::is_integral<T2>::value, int> = 0>
    static PYBIND11_DESCR name()
    {
        return _<std::is_same<T, bool>::value>(
            _("bool"),
            _<std::is_signed<T>::value>("int", "uint") + _<sizeof(T) * 8>());
    }
    template <typename T2 = T,
              enable_if_t<std::is_floating_point<T2>::value, int> = 0>
    static PYBIND11_DESCR name()
    {
        return _ < std::is_same<T, float>::value ||
    ...
    static PYBIND11_DESCR name()
    {
        return _ < std::is_same<typename T2::value_type, float>::value ||
               std::is_same<typename T2::value_type, double>::value >
                   (_("complex") + _<sizeof(typename T2::value_type) * 16>(),
                    _("longcomplex"));
    }
};
...
register_structured_dtype(const std::initializer_list<field_descriptor> &fields,
...
struct npy_format_descriptor
{
    ...
    static void
    register_dtype(const std::initializer_list<field_descriptor> &fields)
    {
        register_structured_dtype(fields,
...
public:
    ...
    : m_shape(shape.size()), m_index(shape.size(), 0), m_common_iterator()
    {
        ...
        for (size_t i = 0; i < N; ++i)
            init_common_iterator(buffers[i], shape, m_common_iterator[i],
                                 strides);
    }
...
// a broadcast_trivial
// enum value indicating whether the broadcast is "trivial"--that is, has each
// buffer being either a
// singleton or a full-size, C-contiguous (`c_trivial`) or Fortran-contiguous
// (`f_trivial`) storage
    ...
    for (size_t i = 0; i < N; ++i)
    {
             ...
             ++shape_iter, ++res_iter)
        {
            ...
            if (dim_size_out == 1)
                ...
                pybind11_fail("pybind11::vectorize: incompatible "
                              "size/dimension of inputs!");
        }
    }
...
struct vectorize_arg
{
    ...
    static constexpr bool vectorize =
        ...
        (!std::is_reference<T>::value || (std::is_lvalue_reference<T>::value &&
                                          std::is_const<call_type>::value));
...
private:
    ...
    static_assert(NVectorized >= 1, "pybind11::vectorize(...) requires a "
                                    "function with at least one vectorizable "
                                    "argument");
...
private:
               ...
               index_sequence<BIndex...> bi_seq)
    {
        ...
        // set here will start
        // out as array_t<T> pointers, but they will be changed them to T
        // pointers before we make
        ...
        if (trivial == broadcast_trivial::non_trivial)
            ...
            apply_trivial(buffers, params, result.mutable_data(), size, i_seq,
                          vi_seq, bi_seq);
                       ...
                       index_sequence<BIndex...>)
    {
        ...
        // references set to the
        // appropriate pointer in `params`; as we iterate, we'll increment each
        // pointer by its size
                         ...
                         index_sequence<BIndex...>)
    {
             ...
             iter != end; ++iter, ++input_iter)
        {
            ...
            *iter = f(*reinterpret_cast<param_n_t<Index> *>(

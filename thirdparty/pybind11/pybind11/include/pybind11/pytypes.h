...
accessors.
    The only requirement for `Derived` is to implement ``PyObject
*Derived::ptr() const``.
\endrst */
...
public:
    ...
    Casting
        the returned ``detail::item_accessor`` instance to a `handle` or
    `object`
        subclass causes a corresponding call to ``__getitem__``. Assigning a
    `handle`
    ...
    the
        returned ``detail::obj_attr_accessor`` instance to a `handle` or
    `object`
        subclass causes a corresponding call to ``getattr``. Assigning a
    `handle`
...
reference
    counting and merely provides a basic C++ interface to various Python API
functions.
...
public:
    ...
    handle(PyObject *ptr) : m_ptr(ptr) {} // Allow implicit conversion from
                                          // PyObject*
    ...
    is
        preferable to use the `object` class which derives from `handle` and
    calls
...
Python
    object (i.e. a ``PyObject *`` in Python's C API). In contrast to `handle`,
it
    optionally increases the object's reference count upon construction, and it
    *always* decreases the reference count when the `object` instance goes out
of
    scope and is destructed. When using `object` instances consistently, it is
much
...
public:
    ...
    /** \rst
        Resets the internal pointer to ``nullptr`` without without decreasing
    the
...
reference.
    The target type ``T`` must be `object` or one of its derived classes. The
function
    doesn't do any conversions or checks. It's up to the user to make sure that
the
...
/// this is typically
/// thrown to propagate python-side errors back through C++ which can either be
/// caught manually or
/// else falls back to the function dispatcher (which then raises the captured
/// error back to
/// python).
...
public:
    ...
    /// currently a Python error
    /// already set it is cleared first.  After this call, the current object no
    /// longer stores the
    ...
    /// exception class (or a
    /// subclass thereof).  May also be passed a tuple to search for any
    /// exception class matches in
    /// the given tuple.
...
// python accessors/methods.
// When given a pyobject, this simply returns the pyobject as-is; for other C++
// type, the value goes
...
public:
    ...
    PYBIND11_DEPRECATED("Use of obj.attr(...) as bool is deprecated in favor "
                        "of pybind11::hasattr(obj, ...)")
    explicit
    operator enable_if_t<
        std::is_same<T, accessor_policies::str_attr>::value ||
    ...
    explicit
    operator enable_if_t<
        std::is_same<T, accessor_policies::generic_item>::value, bool>() const
    {
...
struct sequence_item
{
    ...
    static void set(handle obj, size_t index, handle val)
    {
        ...
        if (PySequence_SetItem(obj.ptr(), static_cast<ssize_t>(index),
                               val.ptr()) != 0)
        {
            throw error_already_set();
        }
...
struct list_item
{
    ...
    {
        PyObject *result =
            PyList_GetItem(obj.ptr(), static_cast<ssize_t>(index));
        if (!result)
        {
            throw error_already_set();
        }
...
struct tuple_item
{
    ...
    {
        PyObject *result =
            PyTuple_GetItem(obj.ptr(), static_cast<ssize_t>(index));
        if (!result)
        {
            throw error_already_set();
        }
...
//       because
//       the `using` statement triggers the parent deprecation warning even if
//       the ctor
//       isn't even used.
...
// back (Unsigned) -1;
// otherwise you get back the unsigned long or unsigned long long value cast to
// (Unsigned).
// (The distinction is critically important when casting a returned -1 error
// value to some other
// unsigned type: (A)-1 != (B)-1 when A and B are unsigned types of different
// sizes).
...
public:
    ...
    {
        m_ptr =
            PyCapsule_New(const_cast<void *>(value), nullptr, [](PyObject *o) {
                auto destructor =
...
public:
    ...
    template <typename... Args, typename = detail::enable_if_t<detail::all_of<
                                    detail::is_keyword_or_ds<Args>...>::value>,
...
private:
    ...
    static PyObject *raw_dict(PyObject *op)
    {
        ...
        return PyObject_CallFunctionObjArgs((PyObject *)&PyDict_Type, op,
                                            nullptr);
    }
};

...
struct container_traits
{
    ...
    static std::true_type test_comparable(
    ...
    enable_if_t<is_comparable<Vector>::value, Class_> &cl)
{
    ...
    cl.def("remove",
           ...
           arg("x"), "Remove the first item from the list whose value is x. "
                ...
                Class_> &cl)
{
    ...
    cl.def("append", [](Vector &v, const T &value) { v.push_back(value); },
...
void vector_accessor(enable_if_t<!vector_needs_copy<Vector>::value, Class_> &cl)
{
    ...
    cl.def(
        ...
        keep_alive<0,
                   1>() /* Essential: keep list alive while iterator exists */
        );
}
...
// format for it
// GCC seems to have "void std::vector<bool>::data()" - doing SFINAE on the
// existence of data() is insufficient, we need to check it returns an
// appropriate pointer
                                  ...
                                  Args &&... args)
{
    ...
    // non-local as well;
    // otherwise (e.g. both types are either module-local or converting) the map
    // will be
    // module-local.
    ...
    cl.def("__bool__", [](const Map &m) -> bool { return !m.empty(); },

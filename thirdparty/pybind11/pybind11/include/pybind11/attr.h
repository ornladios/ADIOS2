...
struct base
{
    PYBIND11_DEPRECATED("base<T>() was deprecated in favor of specifying 'T' "
                        "as a template argument to class_")
    base() {}
};
        ...
        if (!a.value)
        {
            ...
            if (r->is_method)
            {
                if (r->name)
                    ...
                    descr +=
                        " in method of '" + (std::string)str(r->scope) + "'";
            }
            else if (r->name)
            {
...
struct process_attributes
{
    static void init(const Args &... args, function_record *r)
    {
        int unused[] = {
            0,
            (process_attribute<typename std::decay<Args>::type>::init(args, r),
             0)...};
    ...
    static void precall(function_call &call)
    {
        int unused[] = {
            0,
            (process_attribute<typename std::decay<Args>::type>::precall(call),
             0)...};
        ignore_unused(unused);
    }

...
// classes get implicit
// move constructors that break things.  We could detect this an explicitly
...
// the above.  This
// basically covers anything that can be assigned to a dense matrix but that
// don't have a typical
// matrix data layout that can be copied from their .data().  For example,
// DiagonalMatrix and
...
struct EigenConformable
{
    ...
    bool stride_compatible() const
    {
        ...
        // dynamic strides,
        // matching strides, or a dimension size of 1 (in which case the stride
        // value is irrelevant)
...
struct EigenProps
{
    ...
    // Eigen type.  If
    // the array is a vector, we attempt to fit it into either an Eigen 1xN or
    // Nx1 vector
    // (preferring the latter if it will fit in either, i.e. for a fully dynamic
    // matrix type).
    static EigenConformable<row_major> conformable(const array &a)
    {
        ...
        { // Matrix type: require exact match (or dynamic)
            ...
            if ((fixed_rows && np_rows != rows) ||
                (fixed_cols && np_cols != cols))
                return false;
        ...
        else if (fixed_cols)
        {
            ...
            // if it exactly
            // equals the number of elements (rows is Dynamic, and so 1 row is
            // allowed).
            if (cols != n)
                return false;
            return {1, n, stride};
        ...
        {
            // Otherwise it's either fully dynamic, or column dynamic; both
    ...
    {
        constexpr bool show_writeable = is_eigen_dense_map<Type>::value &&
                                        is_eigen_mutable_map<Type>::value;
        constexpr bool show_order = is_eigen_dense_map<Type>::value;
        constexpr bool show_c_contiguous = show_order && requires_row_major;
        constexpr bool show_f_contiguous =
            !show_c_contiguous && show_order && requires_col_major;

        return type_descr(
            _("numpy.ndarray[") + npy_format_descriptor<Scalar>::name() +
            _("[") + _<fixed_rows>(_<(size_t)rows>(), _("m")) + _(", ") +
            _<fixed_cols>(_<(size_t)cols>(), _("n")) + _("]") +
            // For a reference type (e.g. Ref<MatrixXd>) we have other
            // constraints that might need to be
            // satisfied: writeable=True (for a mutable reference), and,
            // depending on the map's stride
            // options, possibly f_contiguous or c_contiguous.  We include them
            // in the descriptor output
            // to provide some hint as to why a TypeError is occurring
            // (otherwise it can be confusing to
            // see that a function accepts a 'numpy.ndarray[float64[3,2]]' and
            // an error message that you
                        ...
                        bool writeable = true)
{
    ...
    array a;
    if (props::vector)
        a = array({src.size()}, {elem_size * src.innerStride()}, src.data(),
                  base);
    else
...
handle eigen_ref_array(Type &src, handle parent = none())
{
    ...
    // currently always
    // copies when there is no base.  Setting the base to None should be
    // harmless.
...
// Eigen Ref/Map classes have slightly different policy requirements, meaning we
// don't want to force
// `move` when a Ref/Map rvalue is returned; we treat Ref<> sort of like a
// pointer (we care about
// the underlying data, not the outer shell).
template <typename Return>
struct return_value_policy_override<
    Return, enable_if_t<is_eigen_dense_map<Return>::value>>
{
...
public:
    ...
    // map/ref points to has
    // to stay around), but we'll allow it under the assumption that you know
    // what you're doing (and
    // have an appropriate keep_alive in place).  We return a numpy array
    // pointing directly at the
    // ref's data (The numpy array ends up read-only if the ref was to a const
    // matrix type.) Note
    // that this means you need to ensure you don't destroy the object in some
    // other way (e.g. with
    // an appropriate keep_alive, or with a reference to a statically allocated
    // matrix).
    ...
    // these can be return
    // types but not bound arguments).  We still provide them (with an
    // explicitly delete) so that
...
private:
    ...
    // source data, but
    // sometimes we can't avoid copying (e.g. input is not a numpy array at all,
    // has an incompatible
    // layout, or is an array of a type that needs to be converted).  Using a
    // numpy temporary
    // (rather than an Eigen temporary) saves an extra copy when we need both
    // type conversion and
    // storage order conversion.  (Note that we refuse to use this temporary
    // copy when loading an
...
public:
    bool load(handle src, bool convert)
    {
        ...
        if (need_copy)
        {
            ...
            // supposed to convert
            // (either because we're in the no-convert overload pass, or because
            // we're explicitly
            // instructed not to copy (via `py::arg().noconvert()`) we have to
            // fail loading.
...
// EigenBase, but not
// EigenDense (i.e. they don't have a data(), at least not with the usual matrix
// layout).
// load() is not supported, but we can cast them into the python domain by first
// copying to a
...
struct type_caster<Type, enable_if_t<is_eigen_sparse<Type>::value>>
{
    ...
    PYBIND11_TYPE_CASTER(
        Type, _<(Type::IsRowMajor) != 0>("scipy.sparse.csr_matrix[",

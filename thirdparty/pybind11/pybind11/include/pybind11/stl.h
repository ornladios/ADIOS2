...
/// type of T (e.g. for
/// forwarding a container element).  Typically used indirect via
/// forwarded_type(), below.
    ...
    static handle cast(T &&src, return_value_policy policy, handle parent)
    {
        ...
        for (auto &&kv : src)
        {
            auto key = reinterpret_steal<object>(
                key_conv::cast(forward_like<T>(kv.first), policy, parent));
            auto value = reinterpret_steal<object>(
                value_conv::cast(forward_like<T>(kv.second), policy, parent));
            if (!key || !value)
                return handle();
            d[key] = value;
        }
...
/// and similar
/// `namespace::variant` types which provide a `namespace::visit()` function are
/// handled here
/// automatically using argument-dependent lookup. Users can provide
...
struct variant_caster<V<Ts...>>
{
    ...
    bool load(handle src, bool convert)
    {
        ...
        // resolution.
        // E.g. `py::int_(1).cast<variant<double, int>>()` needs to fill the
        // `int`
        // slot of the variant. Without two-pass loading `double` would be
        // filled

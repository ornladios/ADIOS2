...
#pragma warning(disable : 1334) // the "template" keyword used for syntactic
// disambiguation may only be used within a
// template
#pragma warning(disable : 1682) // implicit conversion of a 64-bit integral type
// to a smaller integral type (potential
// portability problem)
...
#pragma GCC diagnostic pop
        ...
        using cast_out = detail::make_caster<detail::conditional_t<
            std::is_void<Return>::value, detail::void_type, Return>>;
        ...
        rec->impl = [](detail::function_call &call) -> handle {
             ...
             * rvalue */
            const auto policy =
                detail::return_value_policy_override<Return>::policy(
                    call.func.policy);
        ...
        using detail::descr;
        using detail::_;
        PYBIND11_DESCR signature =
            _("(") + cast_in::arg_names() + _(") -> ") + cast_out::name();

        /* Register the function with Python from generic (non-templated) code
         */
        initialize_generic(rec, signature.text(), signature.types(),
                           sizeof...(Args));
                            ...
                            const std::type_info *const *types, size_t args)
    {
        ...
        for (auto &a : rec->args)
        {
            ...
            if (a.descr)
                ...
                a.descr = strdup(
                    a.value.attr("__repr__")().cast<std::string>().c_str());
        }

        rec->is_constructor = !strcmp(rec->name, "__init__") ||
                              !strcmp(rec->name, "__setstate__");
        ...
        if (rec->is_constructor && !rec->is_new_style_constructor)
        {
            ...
            PyErr_WarnEx(PyExc_FutureWarning,
                         ("pybind11-bound class '" + class_name +
                          ...
                          "placement-new '" +
                          func_name + "' which has been deprecated. See "
                                      "the upgrade guide in pybind11's docs. "
                                      "This message is only visible "
        ...
        while (true)
        {
            ...
            if (c == '{')
            {
                // Write arg name for everything except *args, **kwargs and
                // return type.
                if (type_depth == 0 && text[char_index] != '*' &&
                    arg_index < args)
                {
...
// Rewrite it to the proper class type.
        ...
        if (!chain)
        {
            ...
            m_ptr = PyCFunction_NewEx(rec->def, rec_capsule.ptr(),
                                      scope_module.ptr());
            if (!m_ptr)
                pybind11_fail("cpp_function::cpp_function(): Could not "
        ...
        else
        {
            ...
            if (chain->is_method != rec->is_method)
                pybind11_fail("overloading a method with both static and "
                              "instance methods is not supported; "
#if defined(NDEBUG)
                              "compile in debug mode for more details"
#else
                              "error while attempting to bind " +
                              std::string(rec->is_method ? "instance"
                                                         : "static") +
                              " method " + std::string(pybind11::str(
                                               rec->scope.attr("__name__"))) +
                                ...
                                PyObject *kwargs_in)
    {
        ...
        function_record *overloads = (function_record *)PyCapsule_GetPointer(
                            self, nullptr),
                        *it = overloads;
        ...
        if (overloads->is_constructor)
        {
            ...
            // we really can't support that in C++, so just ignore the second
            // __init__.
        ...
        try
        {
            ...
            // explicit
            // py::arg().noconvert()).  This lets us prefer calls without
            // conversion, with
            ...
            for (; it != nullptr; it = it->next)
            {
                ...
                /* For each overload:
                   ...
                   to make sure that
                      named positional arguments weren't *also* specified via
                   kwarg.
                   2. If we weren't given enough, try to make up the omitted
                   ones by checking
                      whether they were provided by a kwarg matching the
                   `py::arg("name")` name.  If
                      so, use it (and remove it from kwargs; if not, see if the
                   function binding
                   ...
                   py::kwargs, they are an
                      extra tuple or dict at the end of the positional
                   arguments.
                   6. Call the function call dispatcher (function_record::impl)

                   If one of these fail, move on to the next overload and keep
                   trying until we get a
                ...
                if (n_args_in < pos_args && func.args.size() < pos_args)
                    continue; // Not enough arguments given, and not enough
                ...
                if (func.is_new_style_constructor)
                {
                    ...
                    if (self_value_and_holder)
                        self_value_and_holder.type->dealloc(
                ...
                for (; args_copied < args_to_copy; ++args_copied)
                {
                    ...
                    call.args_convert.push_back(arg_rec ? arg_rec->convert
                                                        : true);
                }
                if (bad_arg)
                ...
                if (args_copied < pos_args)
                {
                    ...
                    for (; args_copied < pos_args; ++args_copied)
                    {
                        ...
                        if (kwargs_in && arg.name)
                            value =
                                PyDict_GetItemString(kwargs.ptr(), arg.name);
                ...
                if (func.has_args)
                {
                    ...
                    else
                    {
                        ...
                        {
                            handle item =
                                PyTuple_GET_ITEM(args_in, args_copied + i);
                ...
                if (func.has_kwargs)
                {
                    if (!kwargs.ptr())
                        kwargs =
                            dict(); // If we didn't get one, send an empty one
...
// in `call.args` all along.
                ...
                if (call.args.size() != func.nargs ||
                    ...
                    pybind11_fail("Internal error: function call dispatcher "
                                  "inserted wrong number of arguments!");
#endif
                ...
                if (overloaded)
                {
                    ...
                    // conversion flags for a
                    // set of all-false flags.  If the call fails, we'll swap
                    // the flags back in for
                ...
                if (overloaded)
                {
                    ...
                    // one argument that
                    // permits conversion (i.e. it hasn't been explicitly
                    // specified `.noconvert()`)
                    // then add this call to the list of second pass overloads
                    // to try.
        ...
        catch (...)
        {
            /* When an exception is caught, give each registered exception
               ...
               A translator may choose to do one of the following:
                ...
                - catch the exception and call PyErr_SetString or
               PyErr_SetObject
                  to set a standard (or custom) Python exception, or
                - do nothing and let the exception fall through to the next
               translator, or
                - delegate translation to the next translator by throwing a new
               type of exception. */
        ...
        auto append_note_if_missing_header_is_suspected = [](std::string &msg) {
            if (msg.find("std::") != std::string::npos)
            {
                msg += "\n\n"
                       ...
                       "<pybind11/complex.h>,\n"
                       "<pybind11/functional.h>, <pybind11/chrono.h>, etc. "
                       "Some automatic\n"
                       "conversions are optional and require extra headers to "
                       "be included\n"
        ...
        if (result.ptr() == PYBIND11_TRY_NEXT_OVERLOAD)
        {
            ...
            std::string msg =
                ...
                std::string(overloads->is_constructor ? "constructor"
                                                      : "function") +
                " arguments. The following argument types are supported:\n";

            int ctr = 0;
            for (function_record *it2 = overloads; it2 != nullptr;
                 it2 = it2->next)
            {
                ...
                if (kwargs.size() > 0)
                {
                    ...
                    for (auto kwarg : kwargs)
                    {
                        if (first)
                            ...
                            msg += ", ";
                        msg += pybind11::str("{}={!r}").format(kwarg.first,
                                                               kwarg.second);
                    }
                }
            }
...
public:
    ...
    For
        details on the ``Extra&& ... extra`` argument, see section
    :ref:`extras`.
    \endrst */
    template <typename Func, typename... Extra>
    module &def(const char *name_, Func &&f, const Extra &... extra)
    {
        ...
        // the intention of
        // overwriting (and has already checked internally that it isn't
...
/// execution frame,
/// or ``__main__.__dict__`` if there is no frame (usually when the interpreter
/// is embedded).
inline dict globals()
{
...
protected:
                                  ...
                                  detail::function_record *rec_fget)
    {
        ...
        attr(name) = property(
            ...
            /*deleter*/ none(), pybind11::str(has_doc ? rec_fget->doc : ""));
    }
};
...
public:
    ...
    template <typename Base,
              detail::enable_if_t<!is_base<Base>::value, int> = 0>
                         ...
                         const Extra &... extra)
    {
        ...
        def_property_readonly(
            name, fget, return_value_policy::reference_internal, extra...);
        return *this;
    }
    ...
    {
        return def_property(name, cpp_function(method_adaptor<type>(fget)),
                            fset, return_value_policy::reference_internal,
                            extra...);
    }
...
private:
                            ...
                            const std::enable_shared_from_this<T> * /* dummy */)
    {
        ...
        {
            auto sh =
                std::dynamic_pointer_cast<typename holder_type::element_type>(
                ...
                const void * /* dummy -- not enable_shared_from_this<T>) */)
    {
        if (holder_ptr)
        {
            init_holder_from_existing(
                v_h, holder_ptr, std::is_copy_constructible<holder_type>());
            v_h.set_holder_constructed();
        }
        else if (inst->owned ||
    ...
    /// registering the known
    /// instance.  Should be called as soon as the `type` value_ptr is set for
    /// an instance.  Takes an
    /// optional pointer to an existing holder to use; if not specified and the
    /// instance is
    /// `.owned`, a new holder will be constructed to manage the value pointer.
    static void init_instance(detail::instance *inst, const void *holder_ptr)
    {
        auto v_h =
            inst->get_value_and_holder(detail::get_type_info(typeid(type)));
...
/// when an alias is needed (i.e. due to python-side inheritance).  Arguments
/// must be identical.
...
/// the type
/// returned by `__getstate__` is the same as the argument accepted by
/// `__setstate__`.
...
public:
    ...
    : class_<Type>(scope, name, extra...), m_entries(), m_parent(scope)
    {
        ...
        {
            // Don't provide comparison with the underlying type if the enum
            // isn't convertible,
            // i.e. if Type is a scoped enum, mirroring the C++ behaviour.  (NB:
            // we explicitly
            // convert Type to Scalar below anyway because this needs to
            // compile).
            ...
            if (is_arithmetic)
            {
                ...
                def("__invert__",
                    [](const Type &value) { return ~((Scalar)value); });
 ...
 * exception translator to
 * translate the C++ exception to the created Python exception using the
 * exceptions what() method.
 * This is intended for simple exception translations; for more complex
 * translation, register the
...
public:
    explicit gil_scoped_release(bool disassoc = false) : disassoc(disassoc)
    {
        ...
        // initialize
        // `internals.tstate` for subsequent `gil_scoped_acquire` calls.
        // Otherwise, an
        // initialization race could occur as multiple threads try
        // `gil_scoped_acquire`.

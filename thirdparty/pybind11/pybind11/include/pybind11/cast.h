...
// return value is the pair
// returned by emplace, i.e. an iterator for the entry and a bool set to `true`
// if the entry was
// just created.
...
all_type_info_populate(PyTypeObject *t, std::vector<type_info *> &bases)
{
    ...
    for (size_t i = 0; i < check.size(); i++)
    {
        ...
        if (it != type_dict.end())
        {
            ...
            // or has pre-computed
            // pybind bases, but we have to make sure we haven't already seen
            // the type(s) before: we
            // want to follow Python/virtual C++ rules that there should only be
            // one instance of a
            // common base.
            for (auto *tinfo : it->second)
            {
                ...
                // search, but since
                // having a large number of immediate pybind11-registered types
                // seems fairly
        ...
        else if (type->tp_bases)
        {
            ...
            if (i + 1 == check.size())
            {
                ...
                // avoid growing
                // `check` when adding just one base (which is typical--i.e.
                // when there is no
 ...
 * Python type.  Will
 * be just 1 pybind type for the Python type of a pybind-registered class, or
 * for any Python-side
 * derived class that uses single inheritance.  Will contain as many types as
 * required for a Python
 * class that uses multiple inheritance to inherit (directly or indirectly) from
 * multiple
 * pybind-registered classes.  Will be empty if neither the type nor any base
 * classes are
 * pybind-registered.
 *
 ...
 * neither the type nor any
 * ancestors are pybind11-registered.  Throws an exception if there are multiple
 * bases--use
...
public:
    ...
    public:
        ...
        iterator &operator++()
        {
            ...
            ++curr.index;
            curr.type =
                curr.index < types->size() ? (*types)[curr.index] : nullptr;
            return *this;
        }
 ...
 * contain multiple
 * values/holders for python-side multiple inheritance) that match the given
 * type.  Throws an error
 * if the given type (or ValueType, if omitted) is not a pybind11 base of the
 * given instance.  If
 * `find_type` is omitted (or explicitly specified as nullptr) the first
 * value/holder are returned,
...
PYBIND11_NOINLINE inline void instance::allocate_layout()
{
    ...
    simple_layout =
        n_types == 1 &&
        tinfo.front()->holder_size_in_ptrs <= instance_simple_holder_in_ptrs();
    ...
    { // multiple base types or a too-large holder
        ...
        // value pointer,
        // [hN] is the (uninitialized) holder instance for value N, and [bb...]
        // is a set of bool
        // values that tracks whether each associated holder has been
        // initialized.  Each [block] is
...
// Allocate space for flags, values, and holders, and initialize it to 0 (flags
// and values,
// in particular, need to be 0).  Use Python's memory allocation functions: in
// Python 3.6
// they default to using pymalloc, which is designed to be efficient for small
// allocations
// like the one we're doing here; in earlier versions (and for larger
// allocations) they are
// just wrappers around malloc.
#if PY_VERSION_HEX >= 0x03050000
        nonsimple.values_and_holders =
            (void **)PyMem_Calloc(space, sizeof(void *));
...
#endif
        nonsimple.status = reinterpret_cast<uint8_t *>(
...
public:
         ...
         const void *existing_holder = nullptr)
    {
        ...
        auto it_instances =
            get_internals().registered_instances.equal_range(src);
        for (auto it_i = it_instances.first; it_i != it_instances.second;
             ++it_i)
        {
    ...
    PYBIND11_NOINLINE bool try_load_foreign_module_local(handle src)
    {
        ...
        if (auto result = foreign_typeinfo->module_local_load(src.ptr(),
                                                              foreign_typeinfo))
        {
            value = result;
            return true;
    ...
    // dispatch the relevant
    // bits of code between here and copyable_holder_caster where the two
    // classes need different
    ...
    PYBIND11_NOINLINE bool load_impl(handle src, bool convert)
    {
        ...
        else if (PyType_IsSubtype(srctype, typeinfo->type))
        {
            ...
            // inherits from just
            // one simple (no MI) pybind11 class, or is an exact match, so the
            // C++ instance is of
            // the right type and we can use reinterpret_cast.
            // (This is essentially the same as case 2b, but because not using
            // multiple inheritance
            // is extremely common, we handle it specially to avoid the loop
            // iterator and type
            ...
            // the bases to see if
            // we can find an exact match (or, for a simple C++ type, an
            // inherited match); if so, we
            ...
            // find an exact type match
            // in the registered bases, above, so try implicit casting (needed
            // for proper C++ casting
            // when MI is involved).
    ...
    // dynamic_cast
    // isn't needed or can't be used.  If the type is unknown, sets the error
    // and returns a pair
    // with .second = nullptr.  (p.first = nullptr is not an error: it becomes
    // None).
 ...
 * Such a type caster
 * needs to provide `operator T*()`, `operator T&()`, and `operator T&&() &&`.
 * The latter will be
 * called in appropriate contexts where the value can be moved rather than
 * copied.
 *
...
// similar) through when
// T is non-copyable, but code containing such a copy constructor fails to
// actually compile.
...
// like stl containers
// (we specifically check for: has `value_type` and `reference` with `reference
// = value_type&`): if
// so, copy constructability depends on whether the value_type is copy
// constructible.
...
public:
    ...
    // Returns a (pointer, type_info) pair taking care of necessary RTTI type
    // lookup for a
    // polymorphic type.  If the instance isn't derived, returns the non-RTTI
    // base version.
    template <typename T = itype,
              enable_if_t<std::is_polymorphic<T>::value, int> = 0>
    ...
    src_and_type(const itype *src)
    {
        ...
        if (vsrc)
        {
            ...
            {
                // This is a base pointer to a derived type; if it is a
                // pybind11-registered type, we
                // can get the correct derived pointer (which may be != base
                // pointer) by a
                // dynamic_cast to most derived type:
                if (auto *tpi = get_type_info(*instance_type))
                    return {dynamic_cast<const void *>(src),
                            const_cast<const type_info *>(tpi)};
            }
        }
        // Otherwise we have either a nullptr, an `itype` pointer, or an unknown
        // derived pointer, so
        // don't do a cast
        return type_caster_generic::src_and_type(vsrc, cast_type,
                                                 instance_type);
    }

    // Non-polymorphic type, so no dynamic casting; just call the generic
    // version directly
    template <typename T = itype,
              enable_if_t<!std::is_polymorphic<T>::value, int> = 0>
    static std::pair<const void *, const type_info *>
...
protected:
    ...
    template <typename T,
              typename = enable_if_t<is_copy_constructible<T>::value>>
...
private:
    ...
    static_assert(std::is_same<typename std::remove_const<type>::type &,
                  ...
                  "std::reference_wrapper<T> caster requires T to have a "
                  "caster with an `T &` operator");
...
public:                                                                        \
    ...
    template <typename T_,                                                     \
              enable_if_t<std::is_same<type, remove_cv_t<T_>>::value, int> =   \
                  0>                                                           \
...
template <typename T>
struct type_caster<
    T, enable_if_t<std::is_arithmetic<T>::value && !is_std_char_type<T>::value>>
{
    using _py_type_0 =
...
public:
    bool load(handle src, bool convert)
    {
              ...
              py_value > (py_type)std::numeric_limits<T>::max())))
        {
            ...
            {
                auto tmp =
                    reinterpret_steal<object>(std::is_floating_point<T>::value
...
public:
    bool load(handle src, bool convert)
    {
        ...
        else if (convert || !strcmp("numpy.bool_", Py_TYPE(src.ptr())->tp_name))
        {
...
#if defined(PYPY_VERSION)
            // On PyPy, check that "__bool__" (or "__nonzero__" on Python 2.7)
            // attr exists
            ...
            // but optimized
            // using the CPython API so as to avoid an unneeded attribute
            // lookup.
...
struct string_caster
{
    ...
    bool load(handle src, bool)
    {
        ...
        else if (!PyUnicode_Check(load_src.ptr()))
        {
            ...
            // The below is a guaranteed failure in Python 3 when
            // PyUnicode_Check returns false
            if (!PYBIND11_BYTES_CHECK(load_src.ptr()))
                return false;

            temp =
                reinterpret_steal<object>(PyUnicode_FromObject(load_src.ptr()));
            if (!temp)
            {
                PyErr_Clear();
                return false;
...
private:
    static handle decode_utfN(const char *buffer, ssize_t nbytes)
    {
        ...
        return UTF_N == 8 ? PyUnicode_DecodeUTF8(buffer, nbytes, nullptr)
                          : UTF_N == 16
                                ? PyUnicode_DecodeUTF16(buffer, nbytes, nullptr,
                                                        nullptr)
                                : PyUnicode_DecodeUTF32(buffer, nbytes, nullptr,
                                                        nullptr);
#else
        // PyPy seems to have multiple problems related to PyUnicode_UTF*: the
        // UTF8 version
        // sometimes segfaults for unknown reasons, while the UTF16 and 32
        // versions require a
        // non-const char * arguments, which is also a nuissance, so bypass the
        // whole thing by just
    ...
    // When loading into a std::string or char*, accept a bytes object as-is
    // (i.e.
    // without any encoding/decoding attempt).  For other C++ char sizes this is
    // a no-op.
    ...
    bool load_bytes(enable_if_t<sizeof(C) == 1, handle> src)
    {
        ...
        {
            // We were passed a Python 3 raw bytes; accept it into a std::string
            // or char*
            ...
            {
                value =
                    StringType(bytes, (size_t)PYBIND11_BYTES_SIZE(src.ptr()));
                return true;
            }
        }
...
// but also add the
// ability to use None as a nullptr char* (which the string caster doesn't
// allow).
template <typename CharT>
...
public:
    ...
    operator CharT()
    {
        ...
        // unicode character that
        // is too high, and one for multiple unicode characters (caught later),
        // so we need to figure
        // out how long the first encoded character is in bytes to distinguish
        // between these two
        // errors.  We also allow want to allow unicode characters U+0080
        // through U+00FF, as those
        ...
        // above U+FFFF, thus a
        // surrogate pair with total length 2 instantly indicates a range error
        // (but not a "your
    ...
    using cast_op_type = remove_reference_t<pybind11::detail::cast_op_type<_T>>;
};
...
public:
    ...
    {
        return type_descr(_("Tuple[") +
                          detail::concat(make_caster<Ts>::name()...) + _("]"));
    }
...
/// specializations for
/// custom holders, but it's only necessary if the type has a non-standard
/// interface.
template <typename T>
...
public:
    ...
    bool load(handle src, bool convert)
    {
        return base::template load_impl<
            copyable_holder_caster<type, holder_type>>(src, convert);
    }
...
protected:
    ...
    void check_holder_compat()
    {
        if (typeinfo->default_holder)
            throw cast_error("Unable to load a custom holder type from a "
    ...
    bool load_value(value_and_holder &&v_h)
    {
        ...
        {
            throw cast_error("Unable to cast from non-held to held instance "
                             "(T& to Holder<T>) "
#if defined(NDEBUG)
...
// non-reference/pointer `type`s and reference/pointers from a
// type_caster_generic are safe;
...
// force `policy = move`, regardless of the return value policy the
// function/method was declared
// with.  Some classes (most notably Eigen::Ref and related) need to avoid this,
// and so can do so by
                                  ...
                                  const handle &handle)
{
    ...
    {
#if defined(NDEBUG)
        throw cast_error("Unable to cast Python instance to C++ type (compile "
                         "in debug mode for details)");
#else
        throw cast_error("Unable to cast Python instance of type " +
                         (std::string)str(handle.get_type()) +
                         " to C++ type '" + type_id<T>() + "''");
#endif
    }
    return conv;
...
detail::enable_if_t<!detail::move_never<T>::value, T> move(object &&obj)
{
    if (obj.ref_count() > 1)
#if defined(NDEBUG)
        throw cast_error("Unable to cast Python instance to C++ rvalue: "
                         "instance has multiple references"
                         " (compile in debug mode for details)");
#else
        throw cast_error("Unable to move from Python " +
                         (std::string)str(obj.get_type()) +
...
// going to static_assert, even
// though if it's in dead code, so we provide a "trampoline" to pybind11::cast
// that only does anything in
...
tuple make_tuple(Args &&... args_)
{
    ...
    for (size_t i = 0; i < args.size(); i++)
    {
        ...
        {
#if defined(NDEBUG)
            throw cast_error("make_tuple(): unable to convert arguments to "
                             "Python object (compile in debug mode for "
                             "details)");
#else
...
class argument_loader
{
    ...
    static_assert(args_kwargs_are_last, "py::args/py::kwargs are only "
                                        "permitted as the last argument(s) of "
                                        "a function");
...
/// call.
/// A fancier version below can collect any argument, but this one is optimal
/// for simple calls.
...
private:
    ...
    [[noreturn]] static void nameless_argument_error(std::string type)
    {
        throw type_error("Got kwargs without a name of type '" + type +
                         "'; only named "
                         "arguments may be passed via py::arg() to a python "
                         "function call. ");
    }
...
#define PYBIND11_MAKE_OPAQUE(Type)                                             \
    ...
    {                                                                          \
    template <>                                                                \
    class type_caster<Type> : public type_caster_base<Type>                    \
    {                                                                          \
    };                                                                         \
    }                                                                          \
    }

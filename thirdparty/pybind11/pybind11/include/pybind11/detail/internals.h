...
// libc++ and possibly
// other STLs, this means `typeid(A)` from one module won't equal `typeid(A)`
// from another module
// even when `A` is the same, non-hidden-visibility type (e.g. from a common
// include).  Under
// libstdc++, this doesn't happen: equality and the type_index hash are based on
// the type name,
// which works.  If not under a known-good stl, provide our own name-based hash
// and equality
...
struct internals
{
    ...
    std::vector<PyObject *>
        loader_patient_stack; // Used by `loader_life_support`
    ...
    decltype(PyThread_create_key()) tstate =
        0; // Usually an int but a long on Cygwin64 with Python 3.x
...
// in the
// initial exception translator, below, so add another for our local exception
// classes.
//
...
/// and returns its
/// `c_str()`.  Such strings objects have a long storage duration -- the
/// internal strings are only
/// cleared when the program exits or after interpreter shutdown (when
/// embedding), and so are
/// suitable for c-style strings needed by Python internals (such as
/// PyTypeObject's tp_name).
...
/// the same
/// pybind11 version) running in the current interpreter. Names starting with
/// underscores
/// are reserved for internal usage. Returns `nullptr` if no matching entry was
/// found.
...
/// `get_shared_data()`) if
/// such entry exists. Otherwise, a new object of default-constructible type `T`
/// is
/// added to the shared data under the given name and a reference to it is
/// returned.
template <typename T>

...
// allows types with
// an alias to provide only a single Cpp factory function as long as the Alias
// can be
// constructed from an rvalue reference of the base Cpp type.  This means that
// Alias classes
// can, when appropriate, simply define a `Alias(Cpp &&)` constructor rather
// than needing to
...
// registered class.
// If we don't need an alias (because this class doesn't have one, or because
// the final type is
...
void construct(value_and_holder &v_h, Cpp<Class> *ptr, bool need_alias)
{
    ...
    if (Class::has_alias && need_alias && !is_alias<Class>(ptr))
    {
        ...
        // Whether or not
        // that succeeds, we still need to destroy the original cpp pointer
        // (either the
        // moved away leftover, if the alias construction works, or the value
        // itself if we
        // throw an error), but we can't just call `delete ptr`: it might have a
        // special
        // deleter, or might be shared_from_this.  So we construct a holder
        // around it as if
        // it was a normal instance, then steal the holder away into a local
        // variable; thus
        // the holder and destruction happens when we leave the C++ scope, and
        // the holder
...
// the new instance's
// holder.  This also handles types like std::shared_ptr<T> and
// std::unique_ptr<T> where T is a
// derived type (through those holder's implicit conversion from derived class
// holder constructors).
template <typename Class>
...
// an alias and an
// alias is required the alias must have an `Alias(Cpp &&)` constructor so that
// we can construct
// the alias from the base when needed (i.e. because of Python-side
// inheritance).  When we don't
// need it, we simply move-construct the cpp value into a new instance.
template <typename Class>
void construct(value_and_holder &v_h, Cpp<Class> &&result, bool need_alias)
{
    static_assert(std::is_move_constructible<Cpp<Class>>::value,
                  "pybind11::init() return-by-value factory function requires "
                  "a movable class");
...
// move-construct an
// Alias instance (even if no the python-side inheritance is involved).  The is
// intended for
    ...
    static void execute(Class &cl, const Extra &... extra)
    {
        cl.def("__init__",
               [](value_and_holder &v_h, Args... args) {
                   v_h.value_ptr() =
                       new Cpp<Class>{std::forward<Args>(args)...};
               },
        ...
        cl.def("__init__",
               [](value_and_holder &v_h, Args... args) {
                   if (Py_TYPE(v_h.inst) == v_h.type->type)
                       ...
                       v_h.value_ptr() =
                           new Alias<Class>{std::forward<Args>(args)...};
               },
               is_new_style_constructor(), extra...);
    }

    template <
        typename Class, typename... Extra,
        enable_if_t<Class::has_alias &&
                        !std::is_constructible<Cpp<Class>, Args...>::value,
                    int> = 0>
    static void execute(Class &cl, const Extra &... extra)
...
{
    template <typename Class, typename... Extra,
              enable_if_t<Class::has_alias && std::is_constructible<
                                                  Alias<Class>, Args...>::value,
                          int> = 0>
    static void execute(Class &cl, const Extra &... extra)
    {
        cl.def("__init__",
...
struct factory<Func, void_type (*)(), Return(Args...)>
{
    ...
    // an alias
    // type and an alias instance is needed (i.e. because the final type is a
    // Python class
    // inheriting from the C++ type) the returned value needs to either already
    // be an alias
    // instance, or the alias needs to be constructible from a `Class &&`
    // argument.
    template <typename Class, typename... Extra>
...
struct factory<CFunc, AFunc, CReturn(CArgs...), AReturn(AArgs...)>
{
    static_assert(sizeof...(CArgs) == sizeof...(AArgs),
                  "pybind11::init(class_factory, alias_factory): class and "
                  "alias factories "
    ...
    // the direct
    // class (i.e. not inherited), the alias factory when `self` is a
    // Python-side subtype.
    template <typename Class, typename... Extra>
    void execute(Class &cl, const Extra &... extra) &&
    {
        ...
        cl.def("__init__",
               ...
               (value_and_holder & v_h, CArgs... args) {
                   if (Py_TYPE(v_h.inst) == v_h.type->type)
                       // If the instance type equals the registered type we
                       // don't have inheritance, so
                       // don't need the alias and can construct using the class
                       // function:
                       construct<Class>(
                           v_h, class_func(std::forward<CArgs>(args)...),
                           false);
                   else
                       construct<Class>(

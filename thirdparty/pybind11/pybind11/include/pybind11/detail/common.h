...
// requires forcing hidden visibility on pybind code, so we enforce this by
// setting the attribute on
...
interpreter
    imports a plugin library. Please create a `module` in the function body and
return
...
/** \rst
    This macro creates the entry point that will be invoked when the Python
interpreter
    imports an extension module. The module name is given as the fist argument
and it
    should not be in quotes. The second macro argument defines a variable of
type
 ...
 * a pointer (e.g.  2 means 16 bytes on 64-bit architectures).  The default is
...
struct instance
{
     ...
     * is set with a pointer
     * and the holder object governing that pointer, i.e. [val1*][holder].  This
     * layout is applied
     * whenever there is no python-side multiple inheritance of bound C++ types
     * *and* the type's
     * holder will fit in the default space (which is large enough to hold
     * either a std::unique_ptr
     * or std::shared_ptr).
     *
     * Non-simple layout applies when using custom holders that require more
     * space than `shared_ptr`
     * (which is typically the size of two pointers), or when multiple
     * inheritance is used on the
     * python side.  Non-simple layout allocates the required amount of memory
     * to have multiple
     * bound C++ classes as parents.  Under this layout,
     * `nonsimple.values_and_holders` is set to a
     * pointer to allocated space of the required space to hold a a sequence of
     * value pointers and
     * holders followed `status`, a set of bit flags (1 byte each), i.e.
     * [val1*][holder1][val2*][holder2]...[bb...]  where each [block] is rounded
     * up to a multiple of
     * `sizeof(void *)`.  `nonsimple.holder_constructed` is, for convenience, a
     * pointer to the
    ...
    /// if `find_type`
    /// omitted).  Returns a default-constructed (with `.inst = nullptr`) object
    /// on failure if
...
/// Backports of std::bool_constant and std::negation to accomodate older
/// compilers
template <bool B>
...
struct exactly_one
{
    ...
    static constexpr auto index =
        found ? constexpr_first<Predicate, Ts...>() : 0;
    using type = conditional_t<found, typename pack_element<index, Ts...>::type,
                               Default>;
};
...
/// pointer or member
/// pointer.  Note that this can catch all sorts of other things, too; this is
/// intended to be used
...
#define PYBIND11_EXPAND_SIDE_EFFECTS(PATTERN)                                  \
    pybind11::detail::expand_side_effects                                      \
    {                                                                          \
...
// list in numpy.h
// The order here is: bool; 8 ints ((signed,unsigned)x(8,16,32,64)bits);
// float,double,long double;
// complex float,double,long double.  Note that the long double types only
// participate when long
// double is actually longer than double (it isn't under MSVC).
// NB: not only the string below but also complex.h and numpy.h rely on this
// order.
template <typename T, typename SFINAE = void>
...
// any standard container (or C-style array) supporting std::begin/std::end, any
// singleton
// arithmetic type (if T is arithmetic), or explicitly constructible from an
// iterator pair.
template <typename T>
class any_container
{

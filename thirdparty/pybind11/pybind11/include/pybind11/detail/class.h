...
/** A `static_property` is the same as a `property` but the `__get__()` and
   `__set__()`
    methods are modified to always use the object type instead of a concrete
   instance.
...
/** Types with static properties need to handle `Type.static_prop = x` in a
   specific way.
    By default, Python replaces the `static_property` itself, but for wrapped
   C++ types
    we need to call `static_property.__set__()` in order to propagate the new
   value to
 ...
 * prevents aliasing
 * methods via cls.attr("m2") = cls.attr("m1"): instead the tp_descr_get returns
 * a plain function,
 * when called on a class, or a PyMethod, when called on an instance.  Override
 * that behaviour here
...
/** This metaclass is assigned by default to all pybind11 types and is required
   in order
    for static properties to function correctly. Users may override this using
   `py::metaclass`.
    Return value: New reference. */
...
/// base pointers for any
/// base classes with pointers that are difference from the instance value
/// pointer so that we can
/// correctly recognize an offset base class pointer. This calls a function with
/// any offset base ptrs.
inline void
...
/// instance layout for
/// holding C++ objects and holders.  Allocation is done lazily (the first time
/// the instance is cast
/// to a reference or pointer), and initialization is done by an `__init__`
/// function.
inline PyObject *make_new_instance(PyTypeObject *type)
{
    ...
    // the first inherited
    // object is a a plain Python type (i.e. not derived from an extension
    // type).  Fix it.
...
/// for the
/// C++ object, but doesn't call the constructor -- an `__init__` function must
/// do that.
...
/// least one
/// of these using `py::init` or directly with `.def(__init__, ...)`. Otherwise,
/// the
...
extern "C" inline void pybind11_object_dealloc(PyObject *self)
{
    ...
    // directly
    // with `pybind11_object_dealloc`, but with the common one stashed in
    // internals.
...
inline void enable_dynamic_attributes(PyHeapTypeObject *heap_type)
{
    ...
    static PyGetSetDef getset[] = {
        {const_cast<char *>("__dict__"), pybind11_get_dict, pybind11_set_dict,
         nullptr, nullptr},

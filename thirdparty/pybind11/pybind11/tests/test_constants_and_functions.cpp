/*
    tests/test_constants_and_functions.cpp -- global constants and functions,
   enumerations, raw byte strings
...
// Test that we properly handle C++17 exception specifiers (which are part of
// the function signature
// in C++17).  These should all still work before C++17, but don't affect the
// function signature.
...
TEST_SUBMODULE(constants_and_functions, m)
{
    ...
    m.def("test_function",
          static_cast<py::str (*)(int, float)>(&test_function4));
    m.def("test_function",
          static_cast<py::str (*)(float, int)>(&test_function4));
#endif

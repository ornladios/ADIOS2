...
// Many bases for testing that multiple inheritance from many classes (i.e.
// requiring extra
...
#define PYBIND11_BASEN(N)                                                      \
    py::class_<BaseN<N>>(m, "BaseN" #N)                                        \
        .def(py::init<int>())                                                  \
        .def("f" #N, [](BaseN<N> &b) { return b.i + N; })
    PYBIND11_BASEN(1);
    PYBIND11_BASEN(2);
    PYBIND11_BASEN(3);
    PYBIND11_BASEN(4);
    ...
    // Uncommenting this should result in a compile time failure (MI can only be
    // specified via
    // template parameters because pybind has to know the types involved; see
    // discussion in #742 for
    // details).
    ...
    // Test the case where not all base classes are specified, and where
    // pybind11 requires the
    ...
    m.def("bar_base2a_sharedptr",
          [](std::shared_ptr<Base2a> b) { return b->bar(); });
    ...
    py::class_<I801C, I801B1, I801B2, std::shared_ptr<I801C>>(m, "I801C")
        .def(py::init<>());
    py::class_<I801D, I801C, std::shared_ptr<I801D>>(m, "I801D")
        .def(py::init<>());

    // Two separate issues here: first, we want to recognize a pointer to a base
    // type as being a
    // known instance even when the pointer value is unequal (i.e. due to a
    // non-first
    ...
    // Second, when returned a base class pointer to a derived instance, we
    // cannot assume that the
    // pointer is `reinterpret_cast`able to the derived pointer because, like
    // above, the base class
    ...
    // Return a base class pointer to a pybind-registered type when the actual
    // derived type
    // isn't pybind-registered (and uses multiple-inheritance to offset the
    // pybind base)
    ...
    py::class_<VanillaDictMix1, Vanilla, WithDict>(m, "VanillaDictMix1")
        .def(py::init<>());
    py::class_<VanillaDictMix2, WithDict, Vanilla>(m, "VanillaDictMix2")
        .def(py::init<>());
#endif

    // test_diamond_inheritance
    // Issue #959: segfault when constructing diamond inheritance instance
    // All of these have int members so that there will be various unequal
    // pointers involved.
    struct B
    {
        int b;

...
public:
    ...
    static handle cast(const CopyOnlyInt *src, return_value_policy policy,
                       handle parent)
    {
        if (!src)
            return none().release();
...
TEST_SUBMODULE(copy_move_policies, m)
{
    ...
    m.def("move_tuple",
          [](std::tuple<MoveOnlyInt, MoveOrCopyInt, MoveOnlyInt> t) {
    ...
    m.def("move_copy_nested",
          [](std::pair<MoveOnlyInt,
                       std::pair<std::tuple<MoveOrCopyInt, CopyOnlyInt,
                                            std::tuple<MoveOnlyInt>>,
                                 MoveOrCopyInt>>
                 x) {
              return x.first.value + std::get<0>(x.second.first).value +
                     std::get<1>(x.second.first).value +
                     std::get<0>(std::get<2>(x.second.first)).value +
    ...
    m.def("move_and_copy_cstats", []() {
        ...
        mc.move_assignments = mc.move_constructions = mc.copy_assignments =
            mc.copy_constructions = 0;
        auto &mo = ConstructorStats::get<MoveOnlyInt>();
        mo.move_assignments = mo.move_constructions = mo.copy_assignments =
            mo.copy_constructions = 0;
        auto &co = ConstructorStats::get<CopyOnlyInt>();
        co.move_assignments = co.move_constructions = co.copy_assignments =
            co.copy_constructions = 0;
        py::dict d;
    ...
    m.def("move_optional",
          [](std::optional<MoveOnlyInt> o) { return o->value; });
    m.def("move_or_copy_optional",
          [](std::optional<MoveOrCopyInt> o) { return o->value; });
    m.def("copy_optional",
          [](std::optional<CopyOnlyInt> o) { return o->value; });
    m.def("move_optional_tuple",
          [](std::optional<std::tuple<MoveOrCopyInt, MoveOnlyInt, CopyOnlyInt>>
                 x) {
              return std::get<0>(*x).value + std::get<1>(*x).value +
                     std::get<2>(*x).value;
          });
#else
    ...
    private:
#if defined(_MSC_VER)
#pragma warning(disable : 4822) // warning C4822: local class member function
                                // does not have a body
#endif
        void *operator new(size_t bytes);
    };
    py::class_<PrivateOpNew>(m, "PrivateOpNew")
        .def_readonly("value", &PrivateOpNew::value);
    ...
    m.def("get_moveissue1", [](int i) { return new MoveIssue1(i); },
          py::return_value_policy::move);
    m.def("get_moveissue2", [](int i) { return MoveIssue2(i); },
          py::return_value_policy::move);
}

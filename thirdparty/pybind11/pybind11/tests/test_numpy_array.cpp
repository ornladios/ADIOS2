...
{
    return arr(a.nbytes() - a.offset_at(index...),
...
TEST_SUBMODULE(numpy_array, sm)
{
    ...
    sm.def("isinstance_typed", [](py::object o) {
        return py::isinstance<py::array_t<double>>(o) &&
    ...
    sm.def("overloaded",
           [](py::array_t<unsigned short>) { return "unsigned short"; });
    sm.def("overloaded", [](py::array_t<long long>) { return "long long"; });
    sm.def("overloaded",
           [](py::array_t<std::complex<double>>) { return "double complex"; });
    sm.def("overloaded",
           [](py::array_t<std::complex<float>>) { return "float complex"; });
    ...
    sm.def("overloaded3", [](py::array_t<int>) { return "int"; },
           py::arg().noconvert());
    sm.def("overloaded3", [](py::array_t<double>) { return "double"; },
           py::arg().noconvert());

    // Make sure we don't do unsafe coercion (e.g. float to int) when not using
    // forcecast, but
    // rather that float gets converted via the safe (conversion to double)
    // overload:
    sm.def("overloaded4",
           [](py::array_t<long long, 0>) { return "long long"; });
    sm.def("overloaded4", [](py::array_t<double, 0>) { return "double"; });

    // But we do allow conversion to int if forcecast is enabled (but only if no
    // overload matches
    // without conversion)
    sm.def("overloaded5",
           [](py::array_t<unsigned int>) { return "unsigned int"; });
    ...
    sm.def("array_auxiliaries2",
           [](py::array_t<double> a) { return auxiliaries(a, a); });

    // test_array_failures
    // Issue #785: Uninformative "Unknown internal error" exception when
    // constructing array from empty object:
    ...
    // Issue (unnumbered; reported in #788): regression: initializer lists can
    // be ambiguous
    ...
    sm.def("array_reshape2", [](py::array_t<double> a) {
        ...
        if (dim_sz * dim_sz != a.size())
            throw std::domain_error("array_reshape2: input array total size is "
                                    "not a squared integer");

/*
    tests/test_factory_constructors.cpp -- tests construction from a factory
   function
                                           via py::init_factory()
...
class TestFactory1
{
    ...
    TestFactory1(int v) : value(std::to_string(v))
    {
        print_created(this, value);
    }
    TestFactory1(std::string v) : value(std::move(v))
    {
        print_created(this, value);
    }
...
class TestFactory2
{
    ...
    TestFactory2(int v) : value(std::to_string(v))
    {
        print_created(this, value);
    }
    TestFactory2(std::string v) : value(std::move(v))
    {
        print_created(this, value);
    }
...
protected:
    ...
    TestFactory3(int v) : value(std::to_string(v))
    {
        print_created(this, value);
    }

public:
    TestFactory3(std::string v) : value(std::move(v))
    {
        print_created(this, value);
    }
...
{
public:
    // Special constructor that allows the factory to construct a PyTF6 from a
    // TestFactory6 only
    ...
    int get() override
    {
        PYBIND11_OVERLOAD(int, TestFactory6, get, /*no args*/);
    }
};
...
public:
    ...
    int get() override
    {
        PYBIND11_OVERLOAD(int, TestFactory7, get, /*no args*/);
    }
};
...
#define MAKE_TAG_TYPE(Name)                                                    \
    ...
    py::class_<TestFactory1>(m, "TestFactory1")
        ...
        .def(py::init(
            &TestFactoryHelper::construct1_string)) // raw function pointer
        .def(py::init(
            [](pointer_tag) { return TestFactoryHelper::construct1(); }))
        .def(py::init([](py::handle, int v, py::handle) {
            return TestFactoryHelper::construct1(v);
        }))
    ...
    py::class_<TestFactory3, std::shared_ptr<TestFactory3>>(m, "TestFactory3")
        ...
        .def(py::init(
            [](shared_ptr_tag) { return TestFactoryHelper::construct3(); }))
        .def("__init__",
             [](TestFactory3 &self, std::string v) {
                 new (&self) TestFactory3(v);
             }) // placement-new ctor

        // factories returning a derived type:
        .def(py::init(c4a)) // derived ptr
        .def(py::init(
            [](pointer_tag, TF5_tag, int a) { return new TestFactory5(a); }))
        // derived shared ptr:
        .def(py::init([](shared_ptr_tag, TF4_tag, int a) {
            return std::make_shared<TestFactory4>(a);
        }))
        .def(py::init([](shared_ptr_tag, TF5_tag, int a) {
    ...
    py::class_<TestFactory4, TestFactory3, std::shared_ptr<TestFactory4>>(
        m, "TestFactory4")
        .def(py::init(c4a)) // pointer
        ;

    // Doesn't need to be registered, but registering makes getting
    // ConstructorStats easier:
    py::class_<TestFactory5, TestFactory3, std::shared_ptr<TestFactory5>>(
        m, "TestFactory5");
    ...
    py::class_<TestFactory6, PyTF6>(m, "TestFactory6")
        ...
        .def(py::init(
            [](alias_tag, pointer_tag, int i) { return new PyTF6(i); }))
        .def(py::init(
            [](base_tag, pointer_tag, int i) { return new TestFactory6(i); }))
        .def(py::init([](base_tag, alias_tag, pointer_tag, int i) {
    ...
    py::class_<TestFactory7, PyTF7, std::shared_ptr<TestFactory7>>(
        m, "TestFactory7")
        .def(py::init([](int i) { return TestFactory7(i); },
        ...
        .def(py::init([](shared_ptr_tag, base_tag,
    ...
    // Class with a custom new operator but *without* a placement new operator
    // (issue #948)
    ...
    public:
        ...
        static void *operator new(std::size_t s)
        {
            ...
            py::print("operator new called, returning",
                      reinterpret_cast<uintptr_t>(p));
            return p;
        }
        static void operator delete(void *p)
        {
            py::print("operator delete called on",
                      reinterpret_cast<uintptr_t>(p));
            ::operator delete(p);
        }
        int i;
    ...
    struct NoisyAlloc
    {
        ...
        // MSVC 2015 bug: the above "noisy delete" isn't invoked (fixed in MSVC
        // 2017)
        static void operator delete(void *p)
        {
            py::print("noisy delete");
    ...
    py::class_<NoisyAlloc>(m, "NoisyAlloc")
        // Since these overloads have the same number of arguments, the
        // dispatcher will try each of
        // them until the arguments convert.  Thus we can get a pre-allocation
        // here when passing a
        // single non-integer:
        .def("__init__",
             [](NoisyAlloc *a, int i) {
        ...
        .def("__init__",
             [](NoisyAlloc &a, int i, std::string) { new (&a) NoisyAlloc(i); });

// static_assert testing (the following def's should all fail with appropriate
// compilation errors):
#if 0
    struct BadF1Base {};

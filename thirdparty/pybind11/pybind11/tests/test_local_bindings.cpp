/*
    tests/test_local_bindings.cpp -- tests the py::module_local class feature
   which makes a class
                                     binding local to the module in which it is
   defined.
...
TEST_SUBMODULE(local_bindings, m)
{
    ...
    // The main pybind11 test module is loaded first, so this registration will
    // succeed (the second
    ...
    // py::module_local declarations should be visible across compilation units
    // that get linked together;
    // this tries to register a duplicate local.  It depends on a definition in
    // test_class.cpp and
    // should raise a runtime error from the duplicate definition attempt.  If
    // test_class isn't
    // available it *also* throws a runtime error (with "test_class not enabled"
    // as value).
    m.def("register_local_external", [m]() {
        auto main = py::module::import("pybind11_tests");
        if (py::hasattr(main, "class_"))
        {
            bind_local<LocalExternal, 7>(m, "LocalExternal",
                                         py::module_local());
        }
        else
    ...
    // They can, however, be overridden to global using
    ...
    m.def("register_mixed_global", [m]() {
        bind_local<MixedGlobalLocal, 100>(m, "MixedGlobalLocal",
                                          py::module_local(false));
    });
    m.def("register_mixed_local", [m]() {
        bind_local<MixedLocalGlobal, 1000>(m, "MixedLocalGlobal",
                                           py::module_local());
    });

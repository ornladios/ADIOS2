...
{
    return os << "s:" << v.bool_ << "," << v.uint_ << "," << v.float_ << ","
              << v.ldbl_;
}
...
{
    return os << "p:" << v.bool_ << "," << v.uint_ << "," << v.float_ << ","
              << v.ldbl_;
}
...
py::array_t<int32_t, 0> test_array_ctors(int i)
{
    ...
    case 21:
        return py::array(shape, strides,
                         ptr); // can't have nullptr due to templated ctor
    case 22:
...
py::list test_dtype_ctors()
{
    ...
    list.append(
        py::dtype(py::buffer_info((void *)0, sizeof(unsigned int), "I", 1)));
...
TEST_SUBMODULE(numpy_dtypes, m)
{
    ...
    // typeinfo may be registered before the dtype descriptor for scalar casts
    // to work...
    ...
    // If uncommented, this should produce a static_assert failure telling the
    // user that the struct
    // is not a POD type
    ...
    m.def("create_rec_partial_nested", [](size_t n) {
        py::array_t<PartialNestedStruct, 0> arr =
            mkarray_via_buffer<PartialNestedStruct>(n);
        auto req = arr.request();
    ...
    m.def("create_string_array", [](bool non_empty) {
        py::array_t<StringStruct, 0> arr =
            mkarray_via_buffer<StringStruct>(non_empty ? 4 : 0);
        if (non_empty)
        {
            auto req = arr.request();
        ...
        for (size_t i = 0; i < n; i++)
        {
            ...
            for (size_t j = 0; j < 4; j++)
                for (size_t k = 0; k < 2; k++)
                    ptr[i].d[j][k] =
                        float(i) * 100.0f + float(j) * 10.0f + float(k);
        }
        return arr;
    });
    ...
    m.def("create_complex_array", [](size_t n) {
        py::array_t<ComplexStruct, 0> arr =
            mkarray_via_buffer<ComplexStruct>(n);
    ...
    m.def("compare_buffer_info", []() {
        ...
        list.append(
            py::bool_(py::detail::compare_buffer_info<unsigned>::compare(
        ...
        list.append(py::bool_(py::detail::compare_buffer_info<long>::compare(
            py::buffer_info(nullptr, sizeof(long),
                            sizeof(long) == sizeof(int) ? "i" : "q", 1))));
        list.append(
            py::bool_(py::detail::compare_buffer_info<CompareStruct>::compare(
                py::buffer_info(nullptr, sizeof(CompareStruct),
                                "T{?:x:3xI:y:f:z:}", 1))));
        return list;
    });
    m.def("buffer_to_dtype",
          [](py::buffer &buf) { return py::dtype(buf.request()); });

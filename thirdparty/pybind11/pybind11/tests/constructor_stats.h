...
This header provides a few useful tools for writing examples or tests that want
to check and/or
display object instance lifetimes.  It requires that you include this header and
add the following
...
You can find various examples of these in several of the existing testing .cpp
files.  (Of course
you don't need to add any of the above constructors/operators that you don't
actually have, except
for the destructor).
...
You can also include extra arguments (such as the 100, 200 in the output above,
coming from the
value constructor) for all of the above methods which will be included in the
output.

For testing, each of these also keeps track the created instances and allows you
to check how many
of the various constructors have been invoked from the Python side via code such
as:
...
Note that `.alive()` should usually be the first thing you call as it invokes
Python's garbage
collector to actually destroy objects that aren't yet referenced.

For everything except copy and move constructors and destructors, any extra
values given to the
print_...() function is stored in a class-specific values list which you can
retrieve and inspect
from the ConstructorStats instance `.values()` method.

In some cases, when you need to track instances of a C++ class not registered
with pybind11, you
need to add a function returning the ConstructorStats for the C++ class; this
can be done with:

    m.def("get_special_cstats", &ConstructorStats::get<SpecialClass>,
py::return_value_policy::reference)

Finally, you can suppress the output messages, but keep the constructor tracking
(for
inspection/testing in python) by using the functions with `print_` replaced with
`track_` (e.g.
...
{
protected:
    std::unordered_map<void *, int> _instances; // Need a map rather than set
                                                // because members can shared
                                                // address with parents
    std::list<std::string>
        _values; // Used to track values (e.g. of value constructors)
public:
...
// Force garbage collection to ensure any pending destructors are invoked:
    ...
    static ConstructorStats &get(py::object class_)
    {
        ...
        {
            auto *type_info =
                internals.registered_types_py.at((PyTypeObject *)class_.ptr())
                    .at(0);
        ...
        auto &cs1 = get(*t1);
        // If we have both a t1 and t2 match, one is probably the trampoline
        // class; return whichever
...
// To track construction/destruction, you need to call these methods from the
// various
// constructors/operators.  The ones that take extra values record the given
// values in the
...
void print_constr_details(T *inst, const std::string &action,
                          Output &&... output)
{

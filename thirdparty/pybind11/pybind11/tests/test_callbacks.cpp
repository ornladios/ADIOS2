...
TEST_SUBMODULE(callbacks, m)
{
    ...
    m.def("test_callback2",
          [](py::object func) { return func("Hello", 'x', true, 5); });
    ...
    m.def("test_keyword_args",
          [](py::function f) { return f("x"_a = 10, "y"_a = 20); });
    ...
    m.def("test_arg_conversion_error1",
          [](py::function f) { f(234, UnregisteredType(), "kw"_a = 567); });
    ...
    m.def("test_cleanup", []() -> std::function<void(void)> {
        ...
        return [p]() {
            /* p should be cleaned up when the returned function is garbage
             * collected */
            (void)p;
        };
    });
    ...
    m.def("test_dummy_function",
          [](const std::function<int(int)> &f) -> std::string {
              using fn_type = int (*)(int);
              auto result = f.target<fn_type>();
              if (!result)
              {
                  auto r = f(1);
                  return "can't convert to function pointer: eval(1) = " +
                         std::to_string(r);
              }
              else if (*result == dummy_function)
              {
                  auto r = (*result)(1);
                  return "matches dummy_function: eval(1) = " +
                         std::to_string(r);
              }
              else
              {
                  return "argument does NOT match dummy_function. This should "
                         "never happen!";
              }
          });
    ...
    m.def("func_accepting_func_accepting_base",
          [](std::function<double(AbstractBase &)>) {});

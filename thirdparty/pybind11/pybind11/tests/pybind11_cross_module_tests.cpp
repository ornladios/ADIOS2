/*
    tests/pybind11_cross_module_tests.cpp -- contains tests that require
   multiple modules
...
PYBIND11_MODULE(pybind11_cross_module_tests, m)
{
    ...
    // This registration will fail (global registration when LocalFail is
    // already registered
    // globally in the main test module):
    m.def("register_nonlocal",
          [m]() { bind_local<NonLocalType, 0>(m, "NonLocalType"); });

    // test_stl_bind_local
    // stl_bind.h binders defaults to py::module_local if the types are local or
    // converting:
    py::bind_vector<LocalVec>(m, "LocalVec");
    py::bind_map<LocalMap>(m, "LocalMap");

    // test_stl_bind_global
    // and global if the type (or one of the types, for the map) is global (so
    // these will fail,
    // assuming pybind11_tests is already loaded):
    m.def("register_nonlocal_vec",
          [m]() { py::bind_vector<NonLocalVec>(m, "NonLocalVec"); });
    m.def("register_nonlocal_map",
          [m]() { py::bind_map<NonLocalMap>(m, "NonLocalMap"); });
    // The default can, however, be overridden to global using
    // `py::module_local()` or
    ...
    // We try this both with the global type registered first and vice versa
    // (the order shouldn't
    // matter).
    m.def("register_mixed_global_local", [m]() {
        bind_local<MixedGlobalLocal, 200>(m, "MixedGlobalLocal",
                                          py::module_local());
    });
    m.def("register_mixed_local_global", [m]() {
        bind_local<MixedLocalGlobal, 2000>(m, "MixedLocalGlobal",
                                           py::module_local(false));
    });
    ...
    // test_vector_bool
    // We can't test both stl.h and stl_bind.h conversions of
    // `std::vector<bool>` within
    // the same module (it would be an ODR violation). Therefore `bind_vector`
    // of `bool`
    // is defined here and tested in `test_stl_binders.py`.
    py::bind_vector<std::vector<bool>>(m, "VectorBool");

    // test_missing_header_message
    // The main module already includes stl.h, but we need to test the error
    // message

/*
    tests/test_builtin_casters.cpp -- Casters available without any additional
   headers
...
TEST_SUBMODULE(builtin_casters, m)
{
    ...
    // Some test characters in utf16 and utf32 encodings.  The last one (the 𝐀)
    // contains a null byte
    char32_t a32 = 0x61 /*a*/, z32 = 0x7a /*z*/, ib32 = 0x203d /*‽*/,
             cake32 = 0x1f382 /*🎂*/, mathbfA32 = 0x1d400 /*𝐀*/;
    char16_t b16 = 0x62 /*b*/, z16 = 0x7a, ib16 = 0x203d, cake16_1 = 0xd83c,
             cake16_2 = 0xdf82, mathbfA16_1 = 0xd835, mathbfA16_2 = 0xdc00;
    std::wstring wstr;
    ...
    m.def("good_utf16_string", [=]() {
        return std::u16string(
            {b16, ib16, cake16_1, cake16_2, mathbfA16_1, mathbfA16_2, z16});
    }); // b‽🎂𝐀z
    ...
    // Under Python 2.7, invalid unicode UTF-32 characters don't appear to
    ...
    m.def("ord_char",
          [](char c) -> int { return static_cast<unsigned char>(c); });
...
// test_string_view
    ...
    m.def("string_view_print",
          [](std::string_view s) { py::print(s, s.size()); });
    m.def("string_view16_print",
          [](std::u16string_view s) { py::print(s, s.size()); });
    m.def("string_view32_print",
          [](std::u32string_view s) { py::print(s, s.size()); });
    ...
    m.def("tuple_passthrough",
          [](std::tuple<bool, std::string, int> input) {
              return std::make_tuple(std::get<2>(input), std::get<1>(input),
                                     std::get<0>(input));
          },
    ...
    static std::pair<
        RValueCaster,
        std::tuple<RValueCaster, std::pair<RValueCaster, RValueCaster>>>
        lvnested;
    m.def("rvalue_pair",
    ...
    m.def("lvalue_tuple",
          []() -> const decltype(lvtuple) & { return lvtuple; });
    m.def("rvalue_nested", []() {
        return std::make_pair(
            RValueCaster{},
            std::make_tuple(RValueCaster{},
                            std::make_pair(RValueCaster{}, RValueCaster{})));
    });
    m.def("lvalue_nested",
          []() -> const decltype(lvnested) & { return lvnested; });
    ...
    // test_void_caster
    m.def("load_nullptr_t",
          [](std::nullptr_t) {}); // not useful, but it should still compile
    m.def("cast_nullptr_t", []() { return std::nullptr_t{}; });

    // test_bool_caster
    m.def("bool_passthrough", [](bool arg) { return arg; });
    m.def("bool_passthrough_noconvert", [](bool arg) { return arg; },
          py::arg().noconvert());

    // test_reference_wrapper
    m.def("refwrap_builtin",
          [](std::reference_wrapper<int> p) { return 10 * p.get(); });
    m.def("refwrap_usertype",
          [](std::reference_wrapper<UserType> p) { return p.get().value(); });
    // Not currently supported (std::pair caster has return-by-value cast
    // operator);
    ...
    m.def("complex_cast", [](std::complex<float> x) {

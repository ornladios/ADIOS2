/*
    tests/test_sequences_and_iterators.cpp -- supporting Pythons' sequence
   protocol, iterators,
    etc.
...
py::list test_random_access_iterator(PythonType x)
{
    ...
    checks.append((x.begin() + static_cast<std::ptrdiff_t>(x.size())) ==
                  x.end());
...
TEST_SUBMODULE(sequences_and_iterators, m)
{
    ...
    py::class_<Sequence>(m, "Sequence")
        ...
        .def("__contains__",
             [](const Sequence &s, float v) { return s.contains(v); })
        .def("__reversed__",
             [](const Sequence &s) -> Sequence { return s.reversed(); })
        /// Slicing protocol (optional)
        .def("__getitem__",
             [](const Sequence &s, py::slice slice) -> Sequence * {
        ...
        .def("__setitem__",
             [](Sequence &s, py::slice slice, const Sequence &value) {
                 size_t start, stop, step, slicelength;
                 if (!slice.compute(s.size(), &start, &stop, &step,
                                    &slicelength))
                     throw py::error_already_set();
                 if (slicelength != value.size())
                     throw std::runtime_error("Left and right hand size of "
                                              "slice assignment have different "
                                              "sizes!");
                 for (size_t i = 0; i < slicelength; ++i)
                 {
                     s[start] = value[i];
    ...
    // test_map_iterator
    // Interface of a map-like object that isn't (directly) an unordered_map,
    // but provides some basic
        ...
        .def("nonzero",
             [](const IntPairs &s) {
                 return py::make_iterator(
                     NonZeroIterator<std::pair<int, int>>(s.begin()),
                     NonZeroSentinel());
             },
             py::keep_alive<0, 1>())
        .def("nonzero_keys",
    ...
    m.def("count_none", [](py::object o) {
        return std::count_if(o.begin(), o.end(),
                             [](py::handle h) { return h.is_none(); });
    });
    ...
    m.def("count_nonzeros", [](py::dict d) {
        return std::count_if(d.begin(), d.end(),
                             [](std::pair<py::handle, py::handle> p) {
    ...
    // test_iterator_rvp
    // #388: Can't make iterators via make_iterator() with different r/v
    // policies

...
TEST_SUBMODULE(class_, m)
{
    ...
    py::class_<NoConstructor>(m, "NoConstructor")
        .def_static("new_instance", &NoConstructor::new_instance,
                    "Return an instance");
    ...
    /* One way of declaring a subclass relationship: reference parent's class_
     * object */
    py::class_<Dog>(m, "Dog", pet_class).def(py::init<std::string>());

    /* Another way of declaring a subclass relationship: reference parent's C++
     * type */
    ...
    m.def("return_class_1",
          []() -> BaseClass * { return new DerivedClass1(); });
    m.def("return_class_2",
          []() -> BaseClass * { return new DerivedClass2(); });
    ...
    m.def("mismatched_holder_1", []() {
        ...
        py::class_<MismatchBase1, std::shared_ptr<MismatchBase1>>(
            mod, "MismatchBase1");
    ...
    m.def("implicitly_convert_argument",
          [](const ConvertibleFromUserType &r) { return r.i; });
    m.def("implicitly_convert_variable", [](py::object o) {
        // `o` is `UserType` and `r` is a reference to a temporary created by
        // implicit
        // conversion. This is valid when called inside a bound function because
        // the temp
    ...
    m.add_object("implicitly_convert_variable_fail", [&] {
        auto f = [](PyObject *, PyObject *args) -> PyObject * {
            ...
            try
            { // It should fail here because there is no life support.
        ...
        return py::reinterpret_steal<py::object>(
            PyCFunction_NewEx(def, nullptr, m.ptr()));
    }());
    ...
    py::class_<AliasedHasOpNewDelSize, PyAliasedHasOpNewDelSize> aliased(
        m, "AliasedHasOpNewDelSize");
    aliased.def(py::init<>());
    aliased.attr("size_noalias") = py::int_(sizeof(AliasedHasOpNewDelSize));
    aliased.attr("size_alias") = py::int_(sizeof(PyAliasedHasOpNewDelSize));

    // This test is actually part of test_local_bindings (test_duplicate_local),
    // but we need a
...
typedef py::class_<BreaksBase<1>, std::unique_ptr<BreaksBase<1>>,
                   BreaksTramp<1>>
    DoesntBreak1;
typedef py::class_<BreaksBase<2>, BreaksTramp<2>,
                   std::unique_ptr<BreaksBase<2>>>
    DoesntBreak2;
...
typedef py::class_<BreaksBase<6>, std::shared_ptr<BreaksBase<6>>,
                   BreaksTramp<6>>
    DoesntBreak6;
typedef py::class_<BreaksBase<7>, BreaksTramp<7>,
                   std::shared_ptr<BreaksBase<7>>>
    DoesntBreak7;
typedef py::class_<BreaksBase<8>, std::shared_ptr<BreaksBase<8>>> DoesntBreak8;
#define CHECK_BASE(N)                                                          \
    static_assert(                                                             \
        std::is_same<typename DoesntBreak##N::type, BreaksBase<N>>::value,     \
...
#define CHECK_ALIAS(N)                                                         \
    static_assert(DoesntBreak##N::has_alias &&                                 \
                      std::is_same<typename DoesntBreak##N::type_alias,        \
                                   BreaksTramp<N>>::value,                     \
...
// There's no nice way to test that these fail because they fail to compile;
// leave them here,
// though, so that they can be manually tested by uncommenting them (and seeing
// that compilation
// failures occurs).

// We have to actually look into the type: the typedef alone isn't enough to
// instantiate the type:
#define CHECK_BROKEN(N)                                                        \
    static_assert(                                                             \
        std::is_same<typename Breaks##N::type, BreaksBase<-N>>::value,         \
        "Breaks1 has wrong type!");

//// Two holder classes:
// typedef py::class_<BreaksBase<-1>, std::unique_ptr<BreaksBase<-1>>,
// std::unique_ptr<BreaksBase<-1>>> Breaks1;
// CHECK_BROKEN(1);
//// Two aliases:
// typedef py::class_<BreaksBase<-2>, BreaksTramp<-2>, BreaksTramp<-2>> Breaks2;
// CHECK_BROKEN(2);
//// Holder + 2 aliases
// typedef py::class_<BreaksBase<-3>, std::unique_ptr<BreaksBase<-3>>,
// BreaksTramp<-3>, BreaksTramp<-3>> Breaks3;
// CHECK_BROKEN(3);
//// Alias + 2 holders
// typedef py::class_<BreaksBase<-4>, std::unique_ptr<BreaksBase<-4>>,
// BreaksTramp<-4>, std::shared_ptr<BreaksBase<-4>>> Breaks4;
// CHECK_BROKEN(4);

...
loop:
	...
	double range = 0.000001 * (fabs((double)s1f) + fabs((double)s2f));
	dill_reg	rdf;
	dill_exec_handle h;

	/* reg <- (reg + reg) */
	if (verbose) printf(" - addf\n");
        dill_start_proc(s, "addf",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_addf(s, rdf, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f + s2f);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g addf %g, expected %g, got %g\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	double range = 0.000001 * (fabs((double)s1d) + fabs((double)s2d));
	dill_reg	rdd;
	dill_exec_handle h;

	/* reg <- (reg + reg) */
	if (verbose) printf(" - addd\n");
        dill_start_proc(s, "addd",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_addd(s, rdd, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d + s2d);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g addd %g, expected %g, got %g\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	if (verbose) printf(" - subf\n");
        dill_start_proc(s, "subf",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_subf(s, rdf, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f - s2f);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g subf %g, expected %g, got %g\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg - reg) */
	if (verbose) printf(" - subd\n");
        dill_start_proc(s, "subd",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_subd(s, rdd, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d - s2d);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g subd %g, expected %g, got %g\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	float result;
	float(*func)(float,float);
	double range = 0.000001 * (fabs((double)s1f * (double)s2f));
	dill_reg	rdf;
	dill_exec_handle h;

	/* reg <- (reg * reg) */
	if (verbose) printf(" - mulf\n");
        dill_start_proc(s, "mulf",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_mulf(s, rdf, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f * s2f);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g mulf %g, expected %g, got %g\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	double result;
	double(*func)(double,double);
	double range = 0.000001 * (fabs((double)s1d * (double)s2d));
	dill_reg	rdd;
	dill_exec_handle h;

	/* reg <- (reg * reg) */
	if (verbose) printf(" - muld\n");
        dill_start_proc(s, "muld",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_muld(s, rdd, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d * s2d);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g muld %g, expected %g, got %g\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg / reg) */
	if (verbose) printf(" - divf\n");
        dill_start_proc(s, "divf",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_divf(s, rdf, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f / s2f);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g divf %g, expected %g, got %g\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg / reg) */
	if (verbose) printf(" - divd\n");
        dill_start_proc(s, "divd",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_divd(s, rdd, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d / s2d);
	if ((result > (expected_result + range)) || (result < (expected_result - range))) {
	    printf("Failed test for %g divd %g, expected %g, got %g\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (float) reg */
	if (verbose) printf(" - cvl2f\n");
	while (get_reg_pair(s, DILL_F, &rdf1, DILL_L, &rdl2, reg_set) != 0) {
	    dill_start_proc(s, "cvl2f",  DILL_F, "%l");
	    /* once more, with feeling (after dill_start_proc) */
	    get_reg_pair(s, DILL_F, &rdf1, DILL_L, &rdl2, reg_set++);
	    if (rdl2 == -1) {
	        rdl2 = dill_param_reg(s, 0);
	    } else {
	        dill_movl(s, rdl2, dill_param_reg(s, 0));
	    ...
	    dill_cvl2f(s, rdf1, rdl2);
	    dill_retf(s, rdf1);
	    h = dill_finalize(s);
            func = (float(*)(IMM_TYPE)) dill_get_fp(h);
            expected_result = (float) s1l;
	    result = func(s1l);
	    dill_free_handle(h);
	    if (expected_result != result) {
	        printf("Failed test for cvl2f, reg pair %d, %d, got %g, expected %g for (float) %zx\n", rdf1, rdl2,  result,  expected_result,  s1l);
	        dill_errors++;
	        dill_dump(s);
	    }
	}
	...
	double expected_result;
	double result;
	double(*func)(IMM_TYPE);
	dill_reg	rdd1, rdl2;
	dill_exec_handle h;
	int reg_set = 0;

	/* reg <- (double) reg */
	if (verbose) printf(" - cvl2d\n");
	while (get_reg_pair(s, DILL_D, &rdd1, DILL_L, &rdl2, reg_set) != 0) {
	    dill_start_proc(s, "cvl2d",  DILL_D, "%l");
	    /* once more, with feeling (after dill_start_proc) */
	    get_reg_pair(s, DILL_D, &rdd1, DILL_L, &rdl2, reg_set++);
	    if (rdl2 == -1) {
	        rdl2 = dill_param_reg(s, 0);
	    } else {
	        dill_movl(s, rdl2, dill_param_reg(s, 0));
	    ...
	    dill_cvl2d(s, rdd1, rdl2);
	    dill_retd(s, rdd1);
	    h = dill_finalize(s);
            func = (double(*)(IMM_TYPE)) dill_get_fp(h);
            expected_result = (double) s1l;
	    result = func(s1l);
	    dill_free_handle(h);
	    if (expected_result != result) {
	        printf("Failed test for cvl2d, reg pair %d, %d, got %g, expected %g for (double) %zx\n", rdd1, rdl2,  result,  expected_result,  s1l);
	        dill_errors++;
	        dill_dump(s);
	    }
	}
	...
	/* reg <- (IMM_TYPE) reg */
	if (verbose) printf(" - cvf2l\n");
	while (get_reg_pair(s, DILL_L, &rdl1, DILL_F, &rdf2, reg_set) != 0) {
	    dill_start_proc(s, "cvf2l",  DILL_L, "%f");
	    /* once more, with feeling (after dill_start_proc) */
	    get_reg_pair(s, DILL_L, &rdl1, DILL_F, &rdf2, reg_set++);
	    if (rdf2 == -1) {
	        rdf2 = dill_param_reg(s, 0);
	    } else {
	        dill_movf(s, rdf2, dill_param_reg(s, 0));
	    }

	    dill_cvf2l(s, rdl1, rdf2);
	    dill_retl(s, rdl1);
	    h = dill_finalize(s);
            func = (IMM_TYPE(*)(float)) dill_get_fp(h);
            expected_result = (IMM_TYPE) s1f;
	    result = func(s1f);
	    dill_free_handle(h);
	    if (expected_result != result) {
	        printf("Failed test for cvf2l, reg pair %d, %d, got %zx, expected %zx for (IMM_TYPE) %g\n", rdl1, rdf2,  result,  expected_result,  s1f);
	        dill_errors++;
	        dill_dump(s);
	    }
	}
	...
	/* reg <- (double) reg */
	if (verbose) printf(" - cvf2d\n");
	while (get_reg_pair(s, DILL_D, &rdd1, DILL_F, &rdf2, reg_set) != 0) {
	    dill_start_proc(s, "cvf2d",  DILL_D, "%f");
	    /* once more, with feeling (after dill_start_proc) */
	    get_reg_pair(s, DILL_D, &rdd1, DILL_F, &rdf2, reg_set++);
	    if (rdf2 == -1) {
	        rdf2 = dill_param_reg(s, 0);
	    } else {
	        dill_movf(s, rdf2, dill_param_reg(s, 0));
	    ...
	    dill_cvf2d(s, rdd1, rdf2);
	    dill_retd(s, rdd1);
	    h = dill_finalize(s);
            func = (double(*)(float)) dill_get_fp(h);
            expected_result = (double) s1f;
	    result = func(s1f);
	    dill_free_handle(h);
	    if (expected_result != result) {
	        printf("Failed test for cvf2d, reg pair %d, %d, got %g, expected %g for (double) %g\n", rdd1, rdf2,  result,  expected_result,  s1f);
	        dill_errors++;
	        dill_dump(s);
	    }
	}
	...
	IMM_TYPE expected_result;
	IMM_TYPE result;
	IMM_TYPE(*func)(double);
	dill_reg	rdl1, rdd2;
	dill_exec_handle h;
	int reg_set = 0;

	/* reg <- (IMM_TYPE) reg */
	if (verbose) printf(" - cvd2l\n");
	while (get_reg_pair(s, DILL_L, &rdl1, DILL_D, &rdd2, reg_set) != 0) {
	    dill_start_proc(s, "cvd2l",  DILL_L, "%d");
	    /* once more, with feeling (after dill_start_proc) */
	    get_reg_pair(s, DILL_L, &rdl1, DILL_D, &rdd2, reg_set++);
	    if (rdd2 == -1) {
	        rdd2 = dill_param_reg(s, 0);
	    } else {
	        dill_movd(s, rdd2, dill_param_reg(s, 0));
	    }

	    dill_cvd2l(s, rdl1, rdd2);
	    dill_retl(s, rdl1);
	    h = dill_finalize(s);
            func = (IMM_TYPE(*)(double)) dill_get_fp(h);
            expected_result = (IMM_TYPE) s1d;
	    result = func(s1d);
	    dill_free_handle(h);
	    if (expected_result != result) {
	        printf("Failed test for cvd2l, reg pair %d, %d, got %zx, expected %zx for (IMM_TYPE) %g\n", rdl1, rdd2,  result,  expected_result,  s1d);
	        dill_errors++;
	        dill_dump(s);
	    }
	}
	...
	/* reg <- (float) reg */
	if (verbose) printf(" - cvd2f\n");
	while (get_reg_pair(s, DILL_F, &rdf1, DILL_D, &rdd2, reg_set) != 0) {
	    dill_start_proc(s, "cvd2f",  DILL_F, "%d");
	    /* once more, with feeling (after dill_start_proc) */
	    get_reg_pair(s, DILL_F, &rdf1, DILL_D, &rdd2, reg_set++);
	    if (rdd2 == -1) {
	        rdd2 = dill_param_reg(s, 0);
	    } else {
	        dill_movd(s, rdd2, dill_param_reg(s, 0));
	    ...
	    dill_cvd2f(s, rdf1, rdd2);
	    dill_retf(s, rdf1);
	    h = dill_finalize(s);
            func = (float(*)(double)) dill_get_fp(h);
            expected_result = (float) s1d;
	    result = func(s1d);
	    dill_free_handle(h);
	    if (expected_result != result) {
	        printf("Failed test for cvd2f, reg pair %d, %d, got %g, expected %g for (float) %g\n", rdf1, rdd2,  result,  expected_result,  s1d);
	        dill_errors++;
	        dill_dump(s);
	    }
	}
	...
	/* reg <-   reg */
	if (verbose) printf(" - movf\n");
        dill_start_proc(s, "movf", DILL_F, "%f");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_movf(s, rdf, dill_param_reg(s, 0));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(float)) dill_get_fp(h);
        expected_result =   s1f;
	result = func(s1f);
	dill_free_handle(h);
	if (expected_result != result) {
	    printf("Failed test for movf %g , expected %g, got %g\n",  s1f,  expected_result,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	/* reg <-   reg */
	if (verbose) printf(" - movd\n");
        dill_start_proc(s, "movd", DILL_D, "%d");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_movd(s, rdd, dill_param_reg(s, 0));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(double)) dill_get_fp(h);
        expected_result =   s1d;
	result = func(s1d);
	dill_free_handle(h);
	if (expected_result != result) {
	    printf("Failed test for movd %g , expected %g, got %g\n",  s1d,  expected_result,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	/* reg <- - reg */
	if (verbose) printf(" - negf\n");
        dill_start_proc(s, "negf", DILL_F, "%f");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_negf(s, rdf, dill_param_reg(s, 0));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(float)) dill_get_fp(h);
        expected_result = - s1f;
	...
	dill_free_handle(h);
	if (expected_result != result) {
	    printf("Failed test for negf %g , expected %g, got %g\n",  s1f,  expected_result,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	/* reg <- - reg */
	if (verbose) printf(" - negd\n");
        dill_start_proc(s, "negd", DILL_D, "%d");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_negd(s, rdd, dill_param_reg(s, 0));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(double)) dill_get_fp(h);
        expected_result = - s1d;
	...
	dill_free_handle(h);
	if (expected_result != result) {
	    printf("Failed test for negd %g , expected %g, got %g\n",  s1d,  expected_result,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	/* reg <- (BSWAP) reg */
	if (verbose) printf(" - bswapf\n");
        dill_start_proc(s, "bswap",  DILL_F, "%f");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_bswapf(s, rdf, dill_param_reg(s, 0));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(float)) dill_get_fp(h);
	u.val = s1f;
	...
	dill_free_handle(h);
	if (expected_result != result) {
	    printf("Failed test for bswapf, got %g, expected %g for (float) %g\n",  result,  expected_result,  s1f);
	    u.val = expected_result;
	    printf("expected %x %x ", u.i[0], u.i[1]);
	    u.val = result;
	    printf("got  %x %x \n", u.i[0], u.i[1]);
	...
	/* reg <- (BSWAP) reg */
	if (verbose) printf(" - bswapd\n");
        dill_start_proc(s, "bswap",  DILL_D, "%d");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_bswapd(s, rdd, dill_param_reg(s, 0));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(double)) dill_get_fp(h);
	u.val = s1d;
	...
	dill_free_handle(h);
	if (expected_result != result) {
	    printf("Failed test for bswapd, got %g, expected %g for (double) %g\n",  result,  expected_result,  s1d);
	    u.val = expected_result;
	    printf("expected %x %x ", u.i[0], u.i[1]);
	    u.val = result;
	    printf("got  %x %x \n", u.i[0], u.i[1]);
	...
	if (verbose) printf(" - retf\n");
        dill_start_proc(s, "retf",  DILL_F, "%f");
        	dill_retf(s, dill_param_reg(s, 0));
	h = dill_finalize(s);
        func = (float(*)(float)) dill_get_fp(h);
	result = func(s1f);
	dill_free_handle(h);
	expected_result = s1f;
	if (expected_result != result) {
	    printf("Failed test for retf, expected %g, got %g\n",  expected_result,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	if (verbose) printf(" - retfi\n");
        dill_start_simple_proc(s, "retfi", DILL_F);
        	dill_retfi(s, s1f);
	h = dill_finalize(s);
        func = (float(*)()) dill_get_fp(h);
	result = func();
	dill_free_handle(h);

	if (s1f != result) {
	    printf("Failed test for retfi\n");
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	if (verbose) printf(" - retd\n");
        dill_start_proc(s, "retd",  DILL_D, "%d");
        	dill_retd(s, dill_param_reg(s, 0));
	h = dill_finalize(s);
        func = (double(*)(double)) dill_get_fp(h);
	result = func(s1d);
	dill_free_handle(h);
	expected_result = s1d;
	if (expected_result != result) {
	    printf("Failed test for retd, expected %g, got %g\n",  expected_result,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	if (verbose) printf(" - retdi\n");
        dill_start_simple_proc(s, "retdi", DILL_D);
        	dill_retdi(s, s1d);
	h = dill_finalize(s);
        func = (double(*)()) dill_get_fp(h);
	result = func();
	dill_free_handle(h);

	if (s1d != result) {
	    printf("Failed test for retdi\n");
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	if (verbose) printf(" - stf\n");
	s2ul = (UIMM_TYPE)&df - aligned_offset;

	/* mem [ reg + reg ] <- reg */
        dill_start_proc(s, "stf", DILL_V, "%ul%ul");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

		dill_setf(s, rdf, s1f);
        	dill_stf(s, rdf, dill_param_reg(s, 0), dill_param_reg(s, 1));
	h = dill_finalize(s);
        func = (void(*)(UIMM_TYPE, UIMM_TYPE)) dill_get_fp(h);
        ((void(*)(UIMM_TYPE, UIMM_TYPE))func)(s2ul, aligned_offset);
	dill_free_handle(h);
	if (df != (float)s1f) {
	    printf("Failed test for stf\n");
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	df = 0;
	if (verbose) printf(" - stfi\n");
        dill_start_proc(s, "stfi", DILL_V, "%ul");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

		dill_setf(s, rdf, s1f);
        	dill_stfi(s, rdf, dill_param_reg(s, 0), aligned_offset);
	h = dill_finalize(s);
        func = (void(*)(UIMM_TYPE))dill_get_fp(h);
        ((void(*)(UIMM_TYPE))func)(s2ul);
	dill_free_handle(h);
	...
	if (verbose) printf(" - std\n");
	s2ul = (UIMM_TYPE)&dd - aligned_offset;

	/* mem [ reg + reg ] <- reg */
        dill_start_proc(s, "std", DILL_V, "%ul%ul");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

		dill_setd(s, rdd, s1d);
        	dill_std(s, rdd, dill_param_reg(s, 0), dill_param_reg(s, 1));
	h = dill_finalize(s);
        func = (void(*)(UIMM_TYPE, UIMM_TYPE)) dill_get_fp(h);
        ((void(*)(UIMM_TYPE, UIMM_TYPE))func)(s2ul, aligned_offset);
	dill_free_handle(h);
	if (dd != (double)s1d) {
	    printf("Failed test for std\n");
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	dd = 0;
	if (verbose) printf(" - stdi\n");
        dill_start_proc(s, "stdi", DILL_V, "%ul");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

		dill_setd(s, rdd, s1d);
        	dill_stdi(s, rdd, dill_param_reg(s, 0), aligned_offset);
	h = dill_finalize(s);
        func = (void(*)(UIMM_TYPE))dill_get_fp(h);
        ((void(*)(UIMM_TYPE))func)(s2ul);
	dill_free_handle(h);
    ...
    {
	float (*func)(UIMM_TYPE, UIMM_TYPE);
	float result;
	dill_exec_handle h;
	dill_reg	rdf;

	/* reg <- mem[reg + reg]  */
	if (verbose) printf(" - ldf\n");
        dill_start_proc(s, "ldf", DILL_F, "%ul%ul");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_ldf(s, rdf, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(UIMM_TYPE, UIMM_TYPE)) dill_get_fp(h);
	result = func((UIMM_TYPE)&df - aligned_offset, aligned_offset);
	dill_free_handle(h);
	if (df != result) {
	    printf("Failed test for ldf , expected %g, got %g\n",  df,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }	
    {
	float (*func)(UIMM_TYPE);
	float result;
	dill_reg	rdf;
	dill_exec_handle h;

	/* reg <- mem[reg + imm] */
	if (verbose) printf(" - ldfi\n");
        dill_start_proc(s, "ldfi", DILL_F, "%ul");
		if(!dill_raw_getreg(s, &rdf, DILL_F, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_ldfi(s, rdf, dill_param_reg(s, 0), aligned_offset);
        	dill_retf(s, rdf);
	h = dill_finalize(s);
        func = (float(*)(UIMM_TYPE))dill_get_fp(h);
	result = func((UIMM_TYPE)&df - aligned_offset);
	dill_free_handle(h);
	if (df != result) {
	    printf("Failed test for ldf , expected %g, got %g\n",  df,  result);
	    dill_errors++;
    ...
    {
	double (*func)(UIMM_TYPE, UIMM_TYPE);
	double result;
	dill_exec_handle h;
	dill_reg	rdd;

	/* reg <- mem[reg + reg]  */
	if (verbose) printf(" - ldd\n");
        dill_start_proc(s, "ldd", DILL_D, "%ul%ul");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_ldd(s, rdd, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(UIMM_TYPE, UIMM_TYPE)) dill_get_fp(h);
	result = func((UIMM_TYPE)&dd - aligned_offset, aligned_offset);
	dill_free_handle(h);
	if (dd != result) {
	    printf("Failed test for ldd , expected %g, got %g\n",  dd,  result);
	    dill_errors++;
	    dill_dump(s);
	}
    }	
    {
	double (*func)(UIMM_TYPE);
	double result;
	dill_reg	rdd;
	dill_exec_handle h;

	/* reg <- mem[reg + imm] */
	if (verbose) printf(" - lddi\n");
        dill_start_proc(s, "lddi", DILL_D, "%ul");
		if(!dill_raw_getreg(s, &rdd, DILL_D, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_lddi(s, rdd, dill_param_reg(s, 0), aligned_offset);
        	dill_retd(s, rdd);
	h = dill_finalize(s);
        func = (double(*)(UIMM_TYPE))dill_get_fp(h);
	result = func((UIMM_TYPE)&dd - aligned_offset);
	dill_free_handle(h);
	if (dd != result) {
	    printf("Failed test for ldd , expected %g, got %g\n",  dd,  result);
	    dill_errors++;
	...
	if (verbose) printf(" - beqf\n");
	/* reg <- (reg == reg) */
        dill_start_proc(s, "beqf", DILL_F, "%f%f");
		l = dill_alloc_label(s, NULL);
        	dill_beqf(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
        func = (int (*)(float,float)) dill_get_fp(h);
        di = (s1f == s2f);
	result = func(s1f, s2f);
	dill_free_handle(h);
	if (di != result) {
	    printf("Failed test for beqf\n");
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	if (verbose) printf(" - beqd\n");
	/* reg <- (reg == reg) */
        dill_start_proc(s, "beqd", DILL_D, "%d%d");
		l = dill_alloc_label(s, NULL);
        	dill_beqd(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
        func = (int (*)(double,double)) dill_get_fp(h);
        di = (s1d == s2d);
	result = func(s1d, s2d);
	dill_free_handle(h);
	if (di != result) {
	    printf("Failed test for beqd\n");
	    dill_errors++;
	    dill_dump(s);
	}
    }
	...
	if (verbose) printf(" - bnef\n");
	/* reg <- (reg != reg) */
        dill_start_proc(s, "bnef", DILL_F, "%f%f");
		l = dill_alloc_label(s, NULL);
        	dill_bnef(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bned\n");
	/* reg <- (reg != reg) */
        dill_start_proc(s, "bned", DILL_D, "%d%d");
		l = dill_alloc_label(s, NULL);
        	dill_bned(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bltf\n");
	/* reg <- (reg < reg) */
        dill_start_proc(s, "bltf", DILL_F, "%f%f");
		l = dill_alloc_label(s, NULL);
        	dill_bltf(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bltd\n");
	/* reg <- (reg < reg) */
        dill_start_proc(s, "bltd", DILL_D, "%d%d");
		l = dill_alloc_label(s, NULL);
        	dill_bltd(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - blef\n");
	/* reg <- (reg <= reg) */
        dill_start_proc(s, "blef", DILL_F, "%f%f");
		l = dill_alloc_label(s, NULL);
        	dill_blef(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bled\n");
	/* reg <- (reg <= reg) */
        dill_start_proc(s, "bled", DILL_D, "%d%d");
		l = dill_alloc_label(s, NULL);
        	dill_bled(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bgtf\n");
	/* reg <- (reg > reg) */
        dill_start_proc(s, "bgtf", DILL_F, "%f%f");
		l = dill_alloc_label(s, NULL);
        	dill_bgtf(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bgtd\n");
	/* reg <- (reg > reg) */
        dill_start_proc(s, "bgtd", DILL_D, "%d%d");
		l = dill_alloc_label(s, NULL);
        	dill_bgtd(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bgef\n");
	/* reg <- (reg >= reg) */
        dill_start_proc(s, "bgef", DILL_F, "%f%f");
		l = dill_alloc_label(s, NULL);
        	dill_bgef(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	if (verbose) printf(" - bged\n");
	/* reg <- (reg >= reg) */
        dill_start_proc(s, "bged", DILL_D, "%d%d");
		l = dill_alloc_label(s, NULL);
        	dill_bged(s, dill_param_reg(s, 0),  dill_param_reg(s, 1), l);
        		dill_retii(s, 0);
		dill_mark_label(s, l);
			dill_retii(s, 1);
	h = dill_finalize(s);
	...
	/* reg <- (reg == reg) */
	if (verbose) printf(" - eqf\n");
        dill_start_proc(s, "eqf",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_eqf(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f == s2f);
	if (expected_result != result) {
	    printf("Failed test for %g eqf %g, expected %d, got %d\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg == reg) */
	if (verbose) printf(" - eqd\n");
        dill_start_proc(s, "eqd",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_eqd(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d == s2d);
	if (expected_result != result) {
	    printf("Failed test for %g eqd %g, expected %d, got %d\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg != reg) */
	if (verbose) printf(" - nef\n");
        dill_start_proc(s, "nef",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_nef(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f != s2f);
	if (expected_result != result) {
	    printf("Failed test for %g nef %g, expected %d, got %d\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg != reg) */
	if (verbose) printf(" - ned\n");
        dill_start_proc(s, "ned",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_ned(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d != s2d);
	if (expected_result != result) {
	    printf("Failed test for %g ned %g, expected %d, got %d\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg < reg) */
	if (verbose) printf(" - ltf\n");
        dill_start_proc(s, "ltf",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_ltf(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f < s2f);
	if (expected_result != result) {
	    printf("Failed test for %g ltf %g, expected %d, got %d\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg < reg) */
	if (verbose) printf(" - ltd\n");
        dill_start_proc(s, "ltd",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_ltd(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d < s2d);
	if (expected_result != result) {
	    printf("Failed test for %g ltd %g, expected %d, got %d\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg <= reg) */
	if (verbose) printf(" - lef\n");
        dill_start_proc(s, "lef",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_lef(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f <= s2f);
	if (expected_result != result) {
	    printf("Failed test for %g lef %g, expected %d, got %d\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg <= reg) */
	if (verbose) printf(" - led\n");
        dill_start_proc(s, "led",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_led(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d <= s2d);
	if (expected_result != result) {
	    printf("Failed test for %g led %g, expected %d, got %d\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg > reg) */
	if (verbose) printf(" - gtf\n");
        dill_start_proc(s, "gtf",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_gtf(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f > s2f);
	if (expected_result != result) {
	    printf("Failed test for %g gtf %g, expected %d, got %d\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg > reg) */
	if (verbose) printf(" - gtd\n");
        dill_start_proc(s, "gtd",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_gtd(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d > s2d);
	if (expected_result != result) {
	    printf("Failed test for %g gtd %g, expected %d, got %d\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg >= reg) */
	if (verbose) printf(" - gef\n");
        dill_start_proc(s, "gef",  DILL_F, "%f%f");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_gef(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(float,float)) dill_get_fp(h);
	result = func(s1f, s2f);
	dill_free_handle(h);
        expected_result = (s1f >= s2f);
	if (expected_result != result) {
	    printf("Failed test for %g gef %g, expected %d, got %d\n", s1f, s2f, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}
	...
	/* reg <- (reg >= reg) */
	if (verbose) printf(" - ged\n");
        dill_start_proc(s, "ged",  DILL_D, "%d%d");
		if(!dill_raw_getreg(s, &rdi, DILL_I, DILL_TEMP))
			dill_fatal("out of registers!");

        	dill_ged(s, rdi, dill_param_reg(s, 0), dill_param_reg(s, 1));
        	dill_reti(s, rdi);
	h = dill_finalize(s);
        func = (int(*)(double,double)) dill_get_fp(h);
	result = func(s1d, s2d);
	dill_free_handle(h);
        expected_result = (s1d >= s2d);
	if (expected_result != result) {
	    printf("Failed test for %g ged %g, expected %d, got %d\n", s1d, s2d, expected_result, result);
	    dill_errors++;
	    dill_dump(s);
	}

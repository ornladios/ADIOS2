...
Usage: qemu-binfmt-conf.sh [--qemu-path PATH][--debian][--systemd CPU]
       ...
       --qemu-targets: a list of targets to install for (all)
...
qemu_set_binfmts() {
    if [ -n "${QEMU_TARGETS}" ] ; then
        qemu_target_list="${QEMU_TARGETS}"
    fi
...
options=$(getopt -o ds:Q:S:t:e:hc:p: -l debian,systemd:,qemu-path:,qemu-suffix:,qemu-targets:,exportdir:,help,credential:,persistent: -- "$@")
eval set -- "$options"

...
find source plugins testing examples bindings  -path source/adios2/toolkit/derived/parser/pregen-source -prune -o -regextype posix-extended -iregex '.*\.(h|c|cpp|tcc|cu)' ! -path "source/adios2/toolkit/derived/parser/pregen-source/*" | xargs clang-format -i
DIFF="$(git diff)"
if [ -n "${DIFF}" ]
then
  echo "clang-format:"

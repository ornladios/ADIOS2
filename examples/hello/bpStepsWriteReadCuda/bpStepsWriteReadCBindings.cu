 ...
 *
 * bpStepsWriteReadCBindings.cu  Simple example of writing and reading data through ADIOS2 BP engine with
 * multiple simulations steps for every IO step using CUDA (using the C bindings)
 */
...
void writer(adios2_adios *adios, const char[] fname, const size_t Nx,
            unsigned int nSteps)
{
    ...
    adios2_variable *bpFloats = adios2_define_variable(
            bpIO, "bpFloats", adios2_type_float, 1, shape, start, count,
    ...
    {
        adios2_begin_step(bpWriter, adios2_step_mode_write, 0.0f, &err);
        adios2_set_memory_space(bpFloats, adios2_memory_space_gpu);
        errio = adios2_put(bpWriter, bpFloats, gpuSimData, adios2_mode_sync);
    ...
    errio = adios2_close(bpWriter);
    cudaFree(gpuSimData);
}

void reader(adios2::ADIOS &adios, const std::string &fname, const size_t Nx,
            unsigned int nSteps)
{
    adios2_error errio;
...
int main(int argc, char **argv)
{
    ...
    const char filename[30] = "BP5StepsWriteReadCBindings.bp";

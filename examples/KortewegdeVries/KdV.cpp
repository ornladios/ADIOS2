/*
 * Distributed under the OSI-approved Apache License, Version 2.0.  See accompanying file Copyright.txt for details.
 ...
 * Zabusky, Norman J., and Martin D. Kruskal. "Interaction of solitons in a collisionless plasma and the recurrence of initial states." Physical review letters 15.6 (1965): 240.
 */
#include <iostream>
#include <string>
#include <cmath>
...
void KdV(int64_t N, Real dt, Real t_max, Real delta = 0.022) {
    ...
    auto u_variable = io.DefineVariable<Real>("u", {size_t(N)}, {0}, {size_t(N)}, adios2::ConstantDims);
    ...
    for (int64_t j = 1; j < M - 1; ++j) {
        ...
        Real p = momentum(u2);
        if (std::abs(p) > std::sqrt(std::numeric_limits<Real>::epsilon()) || std::isnan(p)) {
            std::cout << "Solution diverged at t = " << (j+1)*dt << "\n";
            std::cout << "Momentum = " << p << "\n";
            return;
        }
        // The stability condition is very severe this iteration: We have to take way more time steps for stability than we need for accuracy.
        // Hence we need skip a ton of steps so we don't spend all our time writing data:
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) {
        ...
        if (dx_str == "-h" || dx_str == "--help") {
            std::cout << "Usage: ./KdV N t_max δ, where N is number of spacial gridpoints (∆t chosen from ∆x via the stability condition), t_max is max simulation time, and δ is an interaction parameter; e.g., ./KdV 512 10 0.022\n";
            return 0;
        }
        N = std::stoi(dx_str);
    }

...
main(int argc, char **argv)
{
    ...
    cm2 = CManager_create();
    CMfork_comm_thread(cm2);
    target_stone = EValloc_stone(cm2);
    EVassoc_terminal_action(cm2, target_stone, simple_format_list, 
			    simple_handler, &message_count);
    
    stone = EVcreate_thread_bridge_action(cm, cm2, target_stone);

...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (verbose) mexPrintf("Destroy ADIOS object\n");
...
void errorCheck(int nlhs, int nrhs, const mxArray *prhs[]){
    ...
    if ( nrhs != 3 ) {
        mexErrMsgIdAndTxt("MATLAB:adiosclosec:rhs","This function needs exactly 3 arguments: FileHandler, ADIOSHandler, Verbose");
    }
    
    if ( !mxIsUint64(prhs[0]) ) {
        mexErrMsgIdAndTxt("MATLAB:adiosclosec:rhs","First arg must be an uint64 handler to an ADIOS file .");
    } 
    
    if ( !mxIsUint64(prhs[1]) ) {
        mexErrMsgIdAndTxt("MATLAB:adiosclosec:rhs","Second arg must be an uint64 adios object handler.");
    } 

...
public:
    ...
    Variable DefineVariable(const std::string &name, const nb::ndarray<nb::numpy> &array,
                            const Dims &shape, const Dims &start, const Dims &count,
                            const bool isConstantDims);

    Variable DefineVariable(const std::string &name, const nb::object &value, const Dims &shape,
    ...
    Attribute DefineAttribute(const std::string &name, const nb::object &value,
    ...
    Engine Open(const std::string &name, const adios2::Mode openMode);

    Engine Open(const std::string &name, const nb::bytes &metadata);

#if ADIOS2_USE_MPI
    Engine Open(const std::string &name, const adios2::Mode openMode, MPI4PY_Comm comm);
#endif

...
Variable IO::DefineVariable(const std::string &name, const nb::ndarray<nb::numpy> &array,
...
#define declare_type(T)                                                                            \
    else if (array.dtype() == nb::dtype<T>())                                                      \
    {                                                                                              \
...
Variable IO::DefineVariable(const std::string &name, const nb::object &value, const Dims &shape,
                            const Dims &start, const Dims &count, const bool isConstantDims)
{
    helper::CheckForNullptr(m_IO, "for variable " + name + ", in call to IO::DefineVariable");
    core::VariableBase *variable = nullptr;
    const auto tss = nb::cast<std::string>(nb::str(value.type()));
...
Attribute IO::DefineAttribute(const std::string &name, const nb::ndarray<nb::numpy> &array,
...
Attribute IO::DefineAttribute(const std::string &name, const nb::object &value,
                              const std::string &variableName, const std::string separator)
{
    helper::CheckForNullptr(m_IO, "for attribute " + name + ", in call to IO::DefineAttribute");
    ...
    {
        auto v = nb::cast<const int64_t>(value);
        attribute = &m_IO->DefineAttribute(name, v, variableName, separator);
    }
    else if (nb::isinstance<nb::float_>(value))
    {
        auto v = nb::cast<const double>(value);
        attribute = &m_IO->DefineAttribute(name, v, variableName, separator);
    }
    else if (tss == "<class 'complex'>")
    {
        auto v = nb::cast<const std::complex<double>>(value);
        attribute = &m_IO->DefineAttribute(name, v, variableName, separator);
    }
    else if (tss == "<class 'numpy.complex64'>")
    {
        auto v = nb::cast<const std::complex<float>>(value);
        attribute = &m_IO->DefineAttribute(name, v, variableName, separator);
    }
    else if (tss == "<class 'numpy.complex128'>")
    {
...
Engine IO::Open(const std::string &name, const nb::bytes &metadata)
{
    helper::CheckForNullptr(m_IO, "for engine " + name + ", in call to IO::Open");
    return Engine(&m_IO->Open(name, metadata.c_str(), metadata.size()));
}

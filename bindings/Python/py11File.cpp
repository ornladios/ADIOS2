...
File::File(const std::string &name, const std::string mode, const std::string engineType)
: m_Name(name), m_Mode(mode),
  m_Stream(std::make_shared<core::Stream>(name, ToMode(mode), engineType, "Python"))
{
}

File::File(const std::string &name, const std::string mode, const std::string &configFile,
           const std::string ioInConfigFile)
: m_Name(name), m_Mode(mode),
  m_Stream(std::make_shared<core::Stream>(name, ToMode(mode), configFile, ioInConfigFile, "Python"))
{
}
...
#define declare_type(T)                                                                            \
    ...
    {                                                                                              \
        m_Stream->WriteAttribute(name, reinterpret_cast<const T *>(array.data()),                  \
                                 static_cast<size_t>(array.size()), variableName, separator,       \
                                 endStep);                                                         \
    }
...
#undef declare_type
    ...
    {
        throw std::invalid_argument("ERROR: adios2 file write attribute " + name +
...
void File::WriteAttribute(const std::string &name, const std::vector<std::string> &stringArray,
                          const std::string &variableName, const std::string separator,
                          const bool endStep)
{
    m_Stream->WriteAttribute(name, stringArray.data(), stringArray.size(), variableName, separator,
                             endStep);
}

void File::Write(const std::string &name, const pybind11::array &array, const Dims &shape,
...
#define declare_type(T)                                                                            \
    ...
    {                                                                                              \
        m_Stream->Write(name, reinterpret_cast<const T *>(array.data()), shape, start, count,      \
                        vParams(), endStep);                                                       \
    }
...
#undef declare_type
    ...
    {
        throw std::invalid_argument("ERROR: adios2 file write variable " + name +
                                    ", either numpy type is not supported or is not "
                                    "c_style memory contiguous, in call to write\n");
    }
}
...
void File::Write(const std::string &name, const pybind11::array &array, const bool isLocalValue,
                 const bool endStep)
{
    if (isLocalValue)
    {
...
void File::Write(const std::string &name, const std::string &stringValue, const bool isLocalValue,
                 const bool endStep)
{
    m_Stream->Write(name, stringValue, isLocalValue, endStep);
}

bool File::GetStep() const { return const_cast<File *>(this)->m_Stream->GetStep(); }

std::vector<std::string> File::ReadString(const std::string &name, const size_t blockID)
{
    return m_Stream->Read<std::string>(name, blockID);
}

std::vector<std::string> File::ReadString(const std::string &name, const size_t stepStart,
                                          const size_t stepCount, const size_t blockID)
{
    return m_Stream->Read<std::string>(name, Box<size_t>(stepStart, stepCount), blockID);
}
...
pybind11::array File::Read(const std::string &name, const Dims &start, const Dims &count,
                           const size_t blockID)
{
    ...
    {
        const std::string value = m_Stream->Read<std::string>(name, blockID).front();
...
pybind11::array File::ReadAttribute(const std::string &name, const std::string &variableName,
                                    const std::string separator)
{
    const DataType type = m_Stream->m_IO->InquireAttributeType(name, variableName, separator);
...
#define declare_type(T)                                                                            \
    ...
    {                                                                                              \
        core::Attribute<T> *attribute =                                                            \
            m_Stream->m_IO->InquireAttribute<T>(name, variableName, separator);                    \
        ...
        m_Stream->ReadAttribute<T>(name, pyArray.mutable_data(), variableName, separator);         \
        return std::move(pyArray);                                                                 \
    }
    ADIOS2_FOREACH_NUMPY_ATTRIBUTE_TYPE_1ARG(declare_type)
#undef declare_type
    ...
    {
        throw std::invalid_argument("ERROR: adios2 file read attribute " + name +
                                    ", type can't be mapped to a numpy type, in call to read\n");
    }
    return pybind11::array();
}

std::vector<std::string> File::ReadAttributeString(const std::string &name,
                                                   const std::string &variableName,
                                                   const std::string separator)
{
    const core::Attribute<std::string> *attribute =
        m_Stream->m_IO->InquireAttribute<std::string>(name, variableName, separator);
    ...
    m_Stream->ReadAttribute<std::string>(name, data.data(), variableName, separator);
    return data;
}
...
adios2::Mode File::ToMode(const std::string mode) const
{
    ...
    {
        throw std::invalid_argument("ERROR: adios2 mode " + mode + " for file " + m_Name +

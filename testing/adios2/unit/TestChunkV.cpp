...
static void print_array(const std::string name, const uint8_t *ptr, const uint8_t size)
{
...
TEST(ADIOS2ChunkV, ADIOS2ChunkVAlignment)
{
    ...
    /* Make allocation not aligned to either AlignmentSize nor the MemBlockSize*/
    size_t AllocSize = 51;

    {
        ...
        /* First block is alone in first chunk, so only real length (51) should be here */
    ...
    }
    {
        ...
        */
        ...
        /* First chunk must be finalized and aligned to MemBlockSize, so should be 128 now */
        ASSERT_EQ(vec[0].iov_len, 128);
        /* Third block is alone in second chunk, so only real length (51) should be here */

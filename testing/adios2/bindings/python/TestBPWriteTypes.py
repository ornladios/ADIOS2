...
class TestBPWriteTypes(Adios2PythonTestBase):
    def testWriteTypes(self):
        ...
        varI8 = bpIO.DefineVariable(
            "varI8", [size * data.I8.size], [rank * data.I8.size],
            [data.I8.size], True, data.I8.dtype)
        ...
        varU8 = bpIO.DefineVariable(
            "varUI8", [size * data.U8.size], [rank * data.U8.size],
            [data.U8.size], True, data.U8.dtype)
        ...
        varR32 = bpIO.DefineVariable(
            "varR32", [size * data.R32.size], [rank * data.R32.size],
            [data.R32.size], True, data.R32.dtype)

        varR64 = bpIO.DefineVariable(
            "varR64", [size * data.R64.size], [rank * data.R64.size],
            [data.R64.size], True, data.R64.dtype)

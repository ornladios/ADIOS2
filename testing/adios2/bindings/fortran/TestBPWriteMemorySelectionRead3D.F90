program TestBPWriteMemorySelectionRead3D
  ...
  type(adios2_variable), dimension(7) :: vars, vars_in
  ...
  real(kind=16), dimension(:, :, :), allocatable :: data_r16, in_data_r16
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
  allocate (data_r16(nx + 2* ghost_x, ny + 2* ghost_y, nz + 2*ghost_z))
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
  call adios2_define_variable(vars(7), ioPut, 'var_r16', adios2_type_ldp, &
                              3, ishape, istart, icount, &
                              adios2_constant_dims, ierr)
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
  call adios2_set_memory_selection(vars(7), 3, memory_start, memory_count, ierr)
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
    data_r16(ghost_x+1:nx+ghost_x+1,ghost_y+1:ny+ghost_y+1,ghost_z+1:nz+ghost_z+1) = s
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
    call adios2_put(bpWriter, vars(7), data_r16, ierr)
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
  allocate (in_data_r16(nx, ny, isize*nz))
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
    call adios2_inquire_variable(vars_in(7), ioGet, 'var_r16', ierr)
    if( vars_in(7)%valid .eqv. .false. ) stop 'r16 invalid error'
    if( vars_in(7)%name /= 'var_r16' ) stop 'r16 name error'
    if( vars_in(7)%type /= adios2_type_ldp ) stop 'r16 type error'
    if( vars_in(7)%ndims /= 3 ) stop 'r16 dims error'
    call adios2_variable_min(min_r16, vars_in(7), ierr)
    if(min_r16 /= 0 ) stop 'r16 min error'
    call adios2_variable_max(max_r16, vars_in(7), ierr)
    if(max_r16 /= nsteps-1 ) stop 'r16 max error'
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
    call adios2_get(bpReader, 'var_r16', in_data_r16, ierr)
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
          if(in_data_r16(i,j,k) /= current_step) stop 'r16 read error'
#endif
        end do
      end do
    end do

program TestBPWriteMemorySelectionRead2D
  ...
  call adios2_open(bpWriter, ioPut, 'MemSelRW2D_f.bp', adios2_mode_write, ierr)
  ...
  call adios2_open(bpReader, ioGet, 'MemSelRW2D_f.bp', adios2_mode_read, ierr)

  allocate (in_data_i1(nx + 2* ghost_x, ny + 2* ghost_y))
  allocate (in_data_i2(nx + 2* ghost_x, ny + 2* ghost_y))
  allocate (in_data_i4(nx + 2* ghost_x, ny + 2* ghost_y))
  allocate (in_data_i8(nx + 2* ghost_x, ny + 2* ghost_y))
  allocate (in_data_r4(nx + 2* ghost_x, ny + 2* ghost_y))
  allocate (in_data_r8(nx + 2* ghost_x, ny + 2* ghost_y))
  ...
  do
    ...
    ! select slice of current rank
    call adios2_set_selection(vars_in(1), 2, istart, icount, ierr)
    call adios2_set_selection(vars_in(2), 2, istart, icount, ierr)
    call adios2_set_selection(vars_in(3), 2, istart, icount, ierr)
    call adios2_set_selection(vars_in(4), 2, istart, icount, ierr)
    call adios2_set_selection(vars_in(5), 2, istart, icount, ierr)
    call adios2_set_selection(vars_in(6), 2, istart, icount, ierr)

    call adios2_set_memory_selection(vars_in(1), 2, memory_start, memory_count, ierr)
    call adios2_set_memory_selection(vars_in(2), 2, memory_start, memory_count, ierr)
    call adios2_set_memory_selection(vars_in(3), 2, memory_start, memory_count, ierr)
    call adios2_set_memory_selection(vars_in(4), 2, memory_start, memory_count, ierr)
    call adios2_set_memory_selection(vars_in(5), 2, memory_start, memory_count, ierr)
    call adios2_set_memory_selection(vars_in(6), 2, memory_start, memory_count, ierr)
    ...
    ! check inner part
    do j=1+ghost_y,ny+ghost_y
      do i=1+ghost_x,nx+ghost_x
         if(in_data_i1(i,j) /= current_step) then
            write(*,*) 'i1 read error inner part'
            stop 1
         end if
         if(in_data_i2(i,j) /= current_step) then
            write(*,*) 'i2 read error inner part'
            stop 1
         end if
         if(in_data_i4(i,j) /= current_step) then
            write(*,*) 'i4 read error inner part'
            stop 1
         end if
         if(in_data_i8(i,j) /= current_step) then
            write(*,*) 'i8 read rerror inner part'
            stop 1
         end if
         if(in_data_r4(i,j) /= REAL(current_step, 4)) then
            write(*,*) 'r4 read error inner part'
            stop 1
         end if
         if(in_data_r8(i,j) /= REAL(current_step, 8)) then
            write(*,*) 'r8 read rerror inner part'
            stop 1
         end if
      end do
    end do

    ! check for parts of the ghost layer that it has not been touched
    do j=1, ghost_y
      do i=1, nx+2*ghost_x
         if(in_data_i1(i,j) /= -1) then
            write(*,*) 'i1 read error ghost y'
            stop 1
         end if
         if(in_data_i2(i,j) /= -1) then
            write(*,*) 'i2 read error ghost y'
            stop 1
         end if
         if(in_data_i4(i,j) /= -1) then
            write(*,*) 'i4 read error ghost y'
            stop 1
         end if
         if(in_data_i8(i,j) /= -1_8) then
            write(*,*) 'i8 read rerror ghost y'
            stop 1
         end if
         if(in_data_r4(i,j) /= -1.0) then
            write(*,*) 'r4 read error ghost y'
            stop 1
         end if
         if(in_data_r8(i,j) /= -1.0_8) then
            write(*,*) 'r8 read rerror ghost y'
            stop 1
         end if
      end do
    end do
    do j=1, ny+2*ghost_y
      do i=1, ghost_x
         if(in_data_i1(i,j) /= -1) then
            write(*,*) 'i1 read error ghost x'
            stop 1
         end if
         if(in_data_i2(i,j) /= -1) then
            write(*,*) 'i2 read error ghost x'
            stop 1
         end if
         if(in_data_i4(i,j) /= -1) then
            write(*,*) 'i4 read error ghost x'
            stop 1
         end if
         if(in_data_i8(i,j) /= -1_8) then
            write(*,*) 'i8 read rerror ghost x'
            stop 1
         end if
         if(in_data_r4(i,j) /= -1.0) then
            write(*,*) 'r4 read error ghost x'
            stop 1
         end if
         if(in_data_r8(i,j) /= -1.0_8) then
            write(*,*) 'r8 read rerror ghost x'
            stop 1
         end if
      end do
    end do

program TestBPWriteMemorySelectionRead2D
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
  allocate (data_r16(nx + 2* ghost_x, ny + 2* ghost_y))
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
  call adios2_set_memory_selection(vars(7), 2, memory_start, memory_count, ierr)
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
    data_r16(ghost_x+1:nx+ghost_x+1,ghost_y+1:ny+ghost_y+1) = s
#endif
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
  allocate (in_data_r16(nx, isize*ny))
#endif
  ...
  do
    ...
    if( vars_in(7)%ndims /= 2 ) stop 'r16 dims error'
...
#ifdef ADIOS2_HAVE_Fortran_REAL16
          if(in_data_r16(i,j) /= current_step) stop 'r16 read rerror'
#endif
      end do
    end do

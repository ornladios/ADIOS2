...
TEST_F(ADIOSBZip2Wrapper, UInt100)
{
    ...
    const unsigned int bzip2ID =
        var_UInt.AddOperator(adiosBZip2, {{"BlockSize100K", "2"}});

    const std::size_t estimatedSize =
        adiosBZip2.BufferMaxSize(Nx * var_UInt.Sizeof());
    std::vector<char> compressedBuffer(estimatedSize);
    size_t compressedSize = adiosBZip2.Compress(
        myUInts.data(), var_UInt.Count(), var_UInt.Sizeof(), var_UInt.Type(),
        compressedBuffer.data(), var_UInt.OperatorsInfo()[bzip2ID].Parameters);
...
TEST_F(ADIOSBZip2Wrapper, WrongParameterValue)
{
    ...
    const unsigned int bzip2ID =
        var_UInt.AddOperator(adiosBZip2, {{"BlockSize100K", "10"}});
    ...
    EXPECT_THROW(size_t compressedSize = adiosBZip2.Compress(
                     myUInts.data(), var_UInt.Count(), var_UInt.Sizeof(),
                     var_UInt.Type(), compressedBuffer.data(),
                     var_UInt.OperatorsInfo()[bzip2ID].Parameters),

...
TEST_F(BPWriteReadLocalVariablesSelHighLevel, BPWriteReadLocal1DSel)
{
    ...
    // writer
    {
        ...
        for (size_t step = 0; step < NSteps; ++step)
        {
            ...
            // global value
            oStream.write("stepsGlobalValue", step32);
            oStream.write("stepsGlobalValueString", std::to_string(step));

            oStream.write("ranksLocalValue", &mpiRank, {adios2::LocalValueDim});
        }
...
TEST_F(BPWriteReadLocalVariablesSelHighLevel, BPWriteReadLocal2D4x2Sel)
{
    // Each process would write a 1x8 array and all processes would
    // form a mpiSize * Nx 1D array
    ...
    // if (mpiRank == 0)
    {
        ...
        for (size_t b = 0; b < mpiSize; ++b)
        {
            ...
            for (size_t i = 0; i < Nx; ++i)
            {
                ...
                {
                    SmallTestData currentTestData = generateNewSmallTestData(
                        m_TestData, static_cast<int>(s), static_cast<int>(b),
                        mpiSize);

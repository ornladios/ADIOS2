...
TEST_F(BPWriteReadLocalVariablesSelHighLevel, BPWriteReadLocal1DSel)
{
    ...
    // writer
    {
        ...
        for (size_t step = 0; step < NSteps; ++step)
        {
            ...
            oStream.write("stepsGlobalValueString", std::to_string(step),
                          adios2::GlobalValue);
            ...
            oStream.write("i16", currentTestData.I16.data(), shape, start,
                          count);
            oStream.write("i32", currentTestData.I32.data(), shape, start,
                          count);
            oStream.write("i64", currentTestData.I64.data(), shape, start,
                          count);
            oStream.write("u8", currentTestData.U8.data(), shape, start, count);
            oStream.write("u16", currentTestData.U16.data(), shape, start,
                          count);
            oStream.write("u32", currentTestData.U32.data(), shape, start,
                          count);
            oStream.write("u64", currentTestData.U64.data(), shape, start,
                          count);
            oStream.write("r32", currentTestData.R32.data(), shape, start,
                          count);
            oStream.write("r64", currentTestData.R64.data(), shape, start,
                          count);
            oStream.write("cr32", currentTestData.CR32.data(), shape, start,
                          count);
            oStream.write("cr64", currentTestData.CR64.data(), shape, start,
                          count);
            oStream.end_step();
        }
        oStream.close();
        ...
        for (adios2::fstep iStep; adios2::getstep(iStream, iStep);)
        {
            ...
            const std::vector<int32_t> stepsGlobalValue =
                iStep.read<int32_t>("stepsGlobalValue");
            ...
            const std::vector<int32_t> ranksGlobalArray =
                iStep.read<int32_t>("ranksLocalValue");
            ...
            for (size_t b = 0; b < static_cast<size_t>(mpiSize); ++b)
            {
                ...
                for (size_t s = 0; s < Nx; ++s)
                {
                    ...
                    const auto I16 =
                        iStep.read<int16_t>("i16", start, count, b);
                    const auto I32 =
                        iStep.read<int32_t>("i32", start, count, b);
                    const auto I64 =
                        iStep.read<int64_t>("i64", start, count, b);
                    const auto U8 = iStep.read<uint8_t>("u8", start, count, b);
                    const auto U16 =
                        iStep.read<uint16_t>("u16", start, count, b);
                    const auto U32 =
                        iStep.read<uint32_t>("u32", start, count, b);
                    const auto U64 =
                        iStep.read<uint64_t>("u64", start, count, b);
                    const auto R32 = iStep.read<float>("r32", start, count, b);
                    const auto R64 = iStep.read<double>("r64", start, count, b);

                    const auto CR32 = iStep.read<std::complex<float>>(
                        "cr32", start, count, b);
                    const auto CR64 = iStep.read<std::complex<double>>(
                        "cr64", start, count, b);
            ...
            for (size_t b = 0; b < static_cast<size_t>(mpiSize); ++b)
            {
                ...
                for (size_t j = 0; j < Ny; ++j)
                {
                    for (size_t i = 0; i < Nx; ++i)
                    {
                        ...
                        const auto I8 =
                            iStep.read<int8_t>("i8", start, count, b);
                        const auto I16 =
                            iStep.read<int16_t>("i16", start, count, b);
                        const auto I32 =
                            iStep.read<int32_t>("i32", start, count, b);
                        const auto I64 =
                            iStep.read<int64_t>("i64", start, count, b);
                        const auto U8 =
                            iStep.read<uint8_t>("u8", start, count, b);
                        const auto U16 =
                            iStep.read<uint16_t>("u16", start, count, b);
                        const auto U32 =
                            iStep.read<uint32_t>("u32", start, count, b);
                        const auto U64 =
                            iStep.read<uint64_t>("u64", start, count, b);
                        const auto R32 =
                            iStep.read<float>("r32", start, count, b);
                        const auto R64 =
                            iStep.read<double>("r64", start, count, b);
...
TEST_F(BPWriteReadLocalVariablesSelHighLevel, BPWriteReadLocal1DAllStepsSel)
{
    ...
    const std::vector<std::string> stepsGlobalValueString =
        iStream.read<std::string>("stepsGlobalValueString", stepStart,
                                  stepCount);
    ...
    const std::vector<std::string> ranksGlobalArrayString =
        iStream.read<std::string>("ranksLocalValueString", stepStart,
                                  stepCount);
    ...
    for (size_t b = 0; b < static_cast<size_t>(mpiSize); ++b)
    {
        ...
        for (size_t i = 0; i < Nx; ++i)
        {
            ...
            const auto I8 = iStream.read<int8_t>("i8", start, count, stepStart,
                                                 stepCount, b);

            const auto I16 = iStream.read<int16_t>("i16", start, count,
                                                   stepStart, stepCount, b);
            const auto I32 = iStream.read<int32_t>("i32", start, count,
                                                   stepStart, stepCount, b);
            const auto I64 = iStream.read<int64_t>("i64", start, count,
                                                   stepStart, stepCount, b);
            const auto U8 = iStream.read<uint8_t>("u8", start, count, stepStart,
                                                  stepCount, b);
            const auto U16 = iStream.read<uint16_t>("u16", start, count,
                                                    stepStart, stepCount, b);
            const auto U32 = iStream.read<uint32_t>("u32", start, count,
                                                    stepStart, stepCount, b);
            const auto U64 = iStream.read<uint64_t>("u64", start, count,
                                                    stepStart, stepCount, b);
            const auto R32 = iStream.read<float>("r32", start, count, stepStart,
                                                 stepCount, b);
            const auto R64 = iStream.read<double>("r64", start, count,
                                                  stepStart, stepCount, b);

            const auto CR32 = iStream.read<std::complex<float>>(
                "cr32", start, count, stepStart, stepCount, b);
            const auto CR64 = iStream.read<std::complex<double>>(
                "cr64", start, count, stepStart, stepCount, b);

            for (size_t s = 0; s < NSteps; ++s)
            {
                SmallTestData currentTestData =
                    generateNewSmallTestData(m_TestData, static_cast<int>(s),
                                             static_cast<int>(b), mpiSize);
                ...
                for (size_t j = i; j < Nx; ++j)
                {
                    ...
                    ASSERT_EQ(I8[s * (Nx - i) + j - i], currentTestData.I8[j])
                        << msg;
                    ASSERT_EQ(I16[s * (Nx - i) + j - i], currentTestData.I16[j])
                        << msg;
                    EXPECT_EQ(I32[s * (Nx - i) + j - i], currentTestData.I32[j])
                        << msg;
                    EXPECT_EQ(I64[s * (Nx - i) + j - i], currentTestData.I64[j])
                        << msg;

                    EXPECT_EQ(U8[s * (Nx - i) + j - i], currentTestData.U8[j])
                        << msg;
                    EXPECT_EQ(U16[s * (Nx - i) + j - i], currentTestData.U16[j])
                        << msg;
                    EXPECT_EQ(U32[s * (Nx - i) + j - i], currentTestData.U32[j])
                        << msg;
                    EXPECT_EQ(U64[s * (Nx - i) + j - i], currentTestData.U64[j])
                        << msg;
                    EXPECT_EQ(R32[s * (Nx - i) + j - i], currentTestData.R32[j])
                        << msg;
                    EXPECT_EQ(R64[s * (Nx - i) + j - i], currentTestData.R64[j])
                        << msg;

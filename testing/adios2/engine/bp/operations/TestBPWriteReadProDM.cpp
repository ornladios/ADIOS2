...
{
public:
    BPWriteReadProDM() = default;
    virtual void SetUp(){};
    virtual void TearDown(){};
};

TEST_F(BPWriteReadProDM, BPWRProDM1D)
{
    // Refactor a dataset with ProDM, then
    // read back with various accuracies

    int mpiRank = 0, mpiSize = 1;
    const size_t Nx = 30000; // 100k minimum data size for ProDM
    ...
    const std::string fname("BPWRProDM1D_MPI.bp");
#else
    const std::string fname("BPWRProDM1D.bp");
#endif
...
#endif
    {
        ...
        // add operations
        adios2::Operator ProDMOp = adios.DefineOperator("ProDMCompressor", adios2::ops::ProDM);

        var_r32.AddOperation(ProDMOp, {});
        var_r64.AddOperation(ProDMOp, {});
            ...
            for (auto error : errors)
            {
                ...
                assert(accuracyGot32.error <=
                       std::max(accuracyRequested.error,
                                adios2::ops::prodm::FLOAT_ROUNDING_ERROR_LIMIT));
                ...
                assert(accuracyGot64.error <=
                       std::max(accuracyRequested.error,
                                adios2::ops::prodm::DOUBLE_ROUNDING_ERROR_LIMIT));

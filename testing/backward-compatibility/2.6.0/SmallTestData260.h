 ...
 */
#ifndef TESTING_BACKWARD_260_SMALLTESTDATA_H_
#define TESTING_BACKWARD_260_SMALLTESTDATA_H_
...
SmallTestData260 generateNewSmallTestData(SmallTestData260 in, int step,
                                          int rank, int size)
{
...
void UpdateSmallTestData(SmallTestData260 &in, int step, int rank, int size)
{
    int j = rank + 1 + step * size;
    std::for_each(in.I8.begin(), in.I8.end(), [&](int8_t &v) { v += j; });
    std::for_each(in.I16.begin(), in.I16.end(), [&](int16_t &v) { v += j; });
...
#endif // TESTING_BACKWARD_260_SMALLTESTDATA_H_

...
#ifndef ADIOS2_TOOLKIT_BURSTBUFFER_FILEDRAINER_POSIX_H_
#define ADIOS2_TOOLKIT_BURSTBUFFER_FILEDRAINER_POSIX_H_
...
using InputFile = std::shared_ptr<PosixInputFile>;
using OutputFile = std::shared_ptr<PosixOutputFile>;
...
private:
    using InputFileMap = std::map<std::string, InputFile>;
    using OutputFileMap = std::map<std::string, OutputFile>;
    InputFileMap m_InputFileMap;
    OutputFileMap m_OutputFileMap;
    std::string SysErrMsg(const int errorNumber) const;
...
#endif /* ADIOS2_TOOLKIT_BURSTBUFFER_FILEDRAINER_POSIX_H_ */

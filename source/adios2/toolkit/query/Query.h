  ...
  {    
   ...
   class RangeTree
   {
	 ...
	 m_Leaves.push_back(range);
       ...
       void AddNode(RangeTree& node)
       {
	 m_SubNodes.push_back(node);
       }
       

       void SetRelation(adios2::query::Relation r)
       {
	 m_Relation = r;
       }
   ...
   public:
     ...
     virtual void BlockIndexEvaluate(adios2::core::IO&, 
				     adios2::core::Engine&, 
   ...
   public:
     QueryVar(const std::string& varName)
       :m_VarName(varName) {}
     
     std::string& GetVarName() {return m_VarName;} 
     void BlockIndexEvaluate(adios2::core::IO&,
			     adios2::core::Engine&, std::vector<Box<Dims>>& touchedBlocks);
     ...
     { 
       if (box.first.size() != m_Selection.first.size()) return false;
     ...
     {
       m_Selection.first = start; m_Selection.second = count;
     }
   ...
   class QueryComposite: public QueryBase {
    public:
     QueryComposite(adios2::query::Relation relation)
     :m_Relation(relation) 
     {}

  ...
  namespace query {
      ...
      void XmlWorker::ParseIONode(const pugi::xml_node &ioNode) 
	{
	  ...
	  if (m_SourceReader->m_IO.m_Name.compare(ioName.value()) != 0) 
	    throw std::ios_base::failure("invalid query io. Expecting io name = query");
#endif        
	  ...
	  for (const pugi::xml_node &qNameNode : ioNode.children("name"))
	    {
	      const pugi::xml_attribute tag = adios2::query::XmlUtil::XMLAttribute("tag", qNameNode);      
		...
		throw std::ios_base::failure("impactible query found on var:"+q->GetVarName());
	  ...
	  if (qNode == nullptr) {
	    ...
	    const pugi::xml_attribute op = adios2::query::XmlUtil::XMLAttribute("op", qNode);      	    
      ...
      QueryVar* XmlWorker::ParseVarNode(const pugi::xml_node &node,
					adios2::core::IO &currentIO, adios2::core::Engine& reader)
...
#undef declare_type       
	...
	*/
	...
	if (start.size() != count.size()) {
	  throw std::ios_base::failure("dim of startcount does match in the bounding box definition");
	}
	...
	if (!simpleQ.IsSelectionValid(shape)) 
	  throw std::ios_base::failure("invalid selections for selection of var: "+simpleQ.GetVarName());
      }

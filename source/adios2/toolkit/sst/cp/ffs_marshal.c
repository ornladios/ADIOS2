                             ...
                             const size_t *Count, void *Data)
{
    ...
    else
    {
        CP_verbose(Stream, TraceVerbose,
                   "Get request, Name %s, Start %zu, Count %zu\n", Name,
                   Start[0], Count[0]);
...
extern void *SstFFSGetBlocksInfo(SstStream Stream, void *Variable)
{
         ...
         WriterRank++)
    {
        ...
        int ElementSize = VarRec->ElementSize;
        int DimCount = VarRec->DimCount;
        int Type = VarRec->Type;
        void *IncomingData = VarRec->PerWriterIncomingData[WriterRank];
        int FreeIncoming = 0;
...
static struct ControlInfo *BuildControl(SstStream Stream, FMFormat Format)
{
    ...
    {
        ret = realloc(ret,
                      sizeof(*ret) + ControlCount * sizeof(struct ControlInfo));

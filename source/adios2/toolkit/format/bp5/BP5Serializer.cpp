...
BP5Serializer::TimestepInfo BP5Serializer::CloseTimestep(int timestep)
{
    ...
    if (!Info.MetaFormat && Info.MetaFieldCount)
    {
        ...
        FMStructDescRec struct_list[4] = {
            {NULL, NULL, 0, NULL},
            {strdup("complex4"), fcomplex_field_list, sizeof(fcomplex_struct),
             NULL},
            {strdup("complex8"), dcomplex_field_list, sizeof(dcomplex_struct),
             NULL},
            {NULL, NULL, 0, NULL}};
        struct_list[0].format_name = strdup("MetaData");

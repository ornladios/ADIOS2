...
public:
    ...
    IExchange(format::BufferSTL &bufferSTL, const int step) = 0;

    virtual std::vector<std::vector<MPI_Request>>
    IExchangeAbsolutePosition(format::BufferSTL &bufferSTL, const int step) = 0;
    ...
    virtual format::BufferSTL &GetConsumerBuffer(format::BufferSTL &bufferSTL);
...
protected:
    ...
    std::vector<format::BufferSTL> m_Buffers;
};

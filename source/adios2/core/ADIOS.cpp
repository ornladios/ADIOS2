...
#include "adios2/helper/adiosINI.h"
...
/** Process ~/.config/adios2/adios2.ini if it exists */
static INIOptions UserOptions;
const INIOptions &ADIOS::GetUserOptions() { return UserOptions; };
...
#endif
    const std::string cfgFile = homePath + "/.config/adios2/adios2.ini";
    if (adios2sys::SystemTools::FileExists(cfgFile))
    {
        helper::ParseINIFile(m_Comm, cfgFile, UserOptions, homePath);
    }
}

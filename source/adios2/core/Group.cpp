...
void Group::BuildTree()
{
    ...
    for (const auto variablePair : m_IO.m_Variables)
    {
        std::vector<std::string> tokens =
...
std::vector<std::string> Group::AvailableVariables()
{
    ...
    for (auto v : val)
    {
        if (mapPtr->treeMap.find(currentPath + groupDelimiter + v) ==
            mapPtr->treeMap.end())
        {
            if (m_IO.m_Variables.find(currentPath + groupDelimiter + v) !=
                m_IO.m_Variables.end())
            {
...
std::vector<std::string> Group::AvailableAttributes()
{
    ...
    for (auto v : val)
    {
        if (mapPtr->treeMap.find(currentPath + groupDelimiter + v) ==
            mapPtr->treeMap.end())
        {
            if (m_IO.m_Attributes.find(currentPath + groupDelimiter + v) !=
                m_IO.m_Attributes.end())
            {
...
std::string Group::InquireVariableType(const std::string &name) const noexcept
{
    return m_IO.InquireVariableType(currentPath + groupDelimiter + name);
}

std::string Group::InquireVariableType(
    const DataMap::const_iterator itVariable) const noexcept
{
    return m_IO.InquireVariableType(itVariable);
}

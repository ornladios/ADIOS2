    ...
    if (m_IO.m_ArrayOrder == ArrayOrdering::RowMajor)
    {
        while (true)
        {
            int ret = m_Serializer.GetData(
                data, variable.m_Name, variable.m_Start, variable.m_Count, m_CurrentStep,
                variable.GetMemorySpace(data), variable.m_MemoryStart, variable.m_MemoryCount);
            if (ret == 0)
            {
                break;
            }
    ...
    else
    {
        ...
        while (true)
        {
            int ret = m_Serializer.GetData(data, variable.m_Name, start, count, m_CurrentStep,
                                           variable.GetMemorySpace(data), memstart, memcount);
            if (ret == 0)
            {
                break;
            }

...
CamBPWriter::CamBPWriter(IO &io, const std::string &name, const Mode mode,
                     helper::Comm comm)
: Engine("CamBPWriter", io, name, mode, std::move(comm)), m_BP4Serializer(m_Comm),
  ...
  m_FileMetadataIndexManager(m_Comm), m_FileDrainer()
{
    ...
    m_EndMessage = " in call to IO Open CamBPWriter " + m_Name + "\n";
...
#define declare_type(T)                                                        \
    void CamBPWriter::DoPut(Variable<T> &variable,                               \
                          typename Variable<T>::Span &span,                    \
                          const size_t bufferID, const T &value)               \
    {                                                                          \
        TAU_SCOPED_TIMER("CamBPWriter::Put");                                    \
...
#define declare_type(T)                                                        \
    void CamBPWriter::DoPutSync(Variable<T> &variable, const T *data)            \
    {                                                                          \
        PutSyncCommon(variable, variable.SetBlockInfo(data, CurrentStep()));   \
        variable.m_BlocksInfo.pop_back();                                      \
    }                                                                          \
    void CamBPWriter::DoPutDeferred(Variable<T> &variable, const T *data)        \
    {                                                                          \
...
void CamBPWriter::InitParameters()
{
...
void CamBPWriter::InitTransports()
{
...
void CamBPWriter::DoFlush(const bool isFinal, const int transportIndex)
{
...
void CamBPWriter::WriteProfilingJSONFile()
{
    TAU_SCOPED_TIMER("CamBPWriter::WriteProfilingJSONFile");
...
void CamBPWriter::PopulateMetadataIndexFileContent(
...
{
    TAU_SCOPED_TIMER("CamBPWriter::PopulateMetadataIndexFileContent");
...
void CamBPWriter::UpdateActiveFlag(const bool active)
{
...
void CamBPWriter::WriteCollectiveMetadataFile(const bool isFinal)
{

    TAU_SCOPED_TIMER("CamBPWriter::WriteCollectiveMetadataFile");
...
void CamBPWriter::WriteData(const bool isFinal, const int transportIndex)
{
    TAU_SCOPED_TIMER("CamBPWriter::WriteData");
    size_t dataSize;
...
void CamBPWriter::AggregateWriteData(const bool isFinal, const int transportIndex)
{
    TAU_SCOPED_TIMER("CamBPWriter::AggregateWriteData");
...
#define declare_type(T, L)                                                     \
    T *CamBPWriter::DoBufferData_##L(const size_t payloadPosition,               \

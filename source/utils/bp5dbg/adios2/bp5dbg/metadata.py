...
def ReadMetadataStep(f, fileSize, step, WriterCount):
    ...
    totalsize = np.fromfile(f, dtype=np.uint64, count=1)
    print("    TotalMetadata size       : {0}".format(totalsize))
    metadatasizearray = np.fromfile(f, dtype=np.uint64, count=WriterCount)
    attrsizearray = np.fromfile(f, dtype=np.uint64, count=WriterCount)

    for i in range(0, WriterCount):
        print("	Writer " + str(i) + ":  MDsize=" + str(metadatasizearray[i]) +
              ", AttrSize=" + str(attrsizearray))
        f.read(metadatasizearray[i])
        f.read(attrsizearray[i])
...
def DumpMetaData(fileName):
    ...
    with open(fileName, "rb") as f:
        ...
        while (f.tell() < fileSize - 12 and status):
            status = ReadMetadataStep(f, fileSize, step, WriterCount)
            step = step + 1
    return status

...
def SetupArgs():
    ...
    parser.add_argument(
        "command",
        help="Command: create/update/delete",
        choices=["create", "update", "delete", "info"],
    )
    parser.add_argument(
        "--verbose", "-v", help="More verbosity", action="count"
    )
    parser.add_argument("--project", "-p", help="Project name", required=True)
    parser.add_argument("--app", "-a", help="Application name", required=True)
    parser.add_argument("--shot", "-s", help="Shot name", required=True)
    parser.add_argument(
        "--campaign_store",
        "-c",
...
def AddFileToArchive(
    args: dict, filename: str, cur: sqlite3.Cursor, dsID: int
):
    compressed = 1
    try:
        f = open(filename, "rb")
...
def ProcessJsonFile(
    args: dict, jsonlist: list, cur: sqlite3.Cursor, hostID: int, dirID: int
):
...
def AddHostName(longHostName, shortHostName):
    ...
    if row is not None:
        ...
        curHost = cur.execute(
            "insert into host values (?, ?)", (shortHostName, longHostName)
        )
...
def Update(args: dict, cur: sqlite3.Cursor):
    ...
    curDir = cur.execute(
        "insert into directory values (?, ?)", (hostID, rootdir)
    )
...
if __name__ == "__main__":
    ...
    if args.command == "info":
        ...
        if len(jsonFileList) == 0:
            print(
                "There are no campaign data files in  " + args.LocalCampaignDir
            )
            exit(2)
